...
func TestNativeTemplate_ComputeNoCopyApply(t *testing.T) {
	tests := []struct {
		name     string
		ops      []Operation
		expected bool
		...
		{
			name: "leaf function (no escape paths)",
			ops: []Operation{
		...
		{
			name: "SaveContinuation disables optimization",
			ops: []Operation{
				NewOperationSaveContinuationOffsetImmediate(2),
				NewOperationApply(),
				NewOperationRestoreContinuation(),
			},
			expected: false,
		...
		{
			name: "MakeClosure disables optimization",
			ops: []Operation{
		...
		{
			name: "both escape paths present",
			ops: []Operation{
				NewOperationSaveContinuationOffsetImmediate(3),
				NewOperationMakeClosure(),
				NewOperationApply(),
				NewOperationRestoreContinuation(),
			},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			tpl := NewNativeTemplate(1, 0, false, tt.ops...)
			tpl.computeNoCopyApply()
			qt.Assert(t, tpl.NoCopyApply(), qt.Equals, tt.expected)
		})
	}
}

func TestNativeTemplate_Copy_PreservesNoCopyApply(t *testing.T) {
	tpl := NewNativeTemplate(1, 0, false,
		NewOperationLoadVoid(),
		NewOperationRestoreContinuation(),
	)
	tpl.computeNoCopyApply()
	qt.Assert(t, tpl.NoCopyApply(), qt.IsTrue)
