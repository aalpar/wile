...
// (slot setup without binding value copy) that happens on every non-tail
// MachineClosure call. fib's template has noCopy=false (it contains
// SaveContinuation), so every recursive call pays this cost.
