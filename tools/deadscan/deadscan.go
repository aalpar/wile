...
// Package deadscan reports exported symbols that no production code consumes.
//
// It is the analysis half of tools/cmd/deadscan, split out so that wile-goast
// can join the census against its own call-graph and CFG analyses. Everything
// here returns data; nothing prints.
//
// It exists because `deadcode` does not run on this toolchain and `unused`
// skips exported identifiers outright, so the exported surface is the one place
// nothing measures. Its command is a REPORTER, not a gate: it exits 0 on a
// successful scan and is not wired into `make lint`.
//
...
// # Usage
//
//	r, err := deadscan.Load(".", []string{"github.com/aalpar/wile/..."})
//
// Pass every workspace module whose consumers should count, or the ext column
// is zero and the dead list is overstated. Result.Modules reports what was
// actually loaded, so that mistake is visible rather than silent.
package deadscan
...
// Result is one scan: every audited exported symbol, and the main modules the
// scan actually loaded.
//
// Modules is reported because it is the one input a caller can get wrong
// silently. Load every workspace module whose references should count, or the
// ExtBy column is empty and the dead list is overstated.
type Result struct {
	Symbols []*Symbol
	Modules []string
}

// Load type-checks the given package patterns and returns the census.
//
// dir is the directory the patterns resolve against; patterns are the
// go/packages patterns to load, e.g. "github.com/aalpar/wile/...". Tests are
// loaded too: a reference from a _test.go file is classified test rather than
// production, and that distinction is what the whole census turns on.
func Load(dir string, patterns []string) (*Result, error) {
	...
	return &Result{Symbols: p.sorted(), Modules: sortedKeys(p.modules)}, nil
}
