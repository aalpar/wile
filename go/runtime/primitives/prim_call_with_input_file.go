...
// Opens a file for reading, calls the procedure with the port, then closes the port.
...
func PrimCallWithInputFile(ctx context.Context, mc *machine.MachineContext) error {
	...
	if !ok {
		return values.WrapForeignErrorf(values.ErrNotAString, "call-with-input-file: expected a string but got %T", filenameVal)
	}

	proc, ok := procVal.(*machine.MachineClosure)
	if !ok {
		return values.WrapForeignErrorf(values.ErrNotAProcedure, "call-with-input-file: expected a procedure but got %T", procVal)
	}

	// Open the file
	file, err := os.Open(filename.Value)
	if err != nil {
		return values.WrapForeignErrorf(err, "call-with-input-file: %v", err)
	}
