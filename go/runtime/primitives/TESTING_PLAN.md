...
| Type | Go Type | Constructor | Example Scheme Literal |
|------|---------|-------------|------------------------|
| Integer | `*values.Integer` | `values.NewInteger(42)` | `42` |
| BigInteger | `*values.BigInteger` | `values.NewBigIntegerFromString("123456789012345678901234567890", 10)` | `#z123456789012345678901234567890` |
| Float | `*values.Float` | `values.NewFloat(3.14)` | `3.14` |
| Rational | `*values.Rational` | `values.NewRational(3, 4)` | `3/4` |
| Complex | `*values.Complex` | `values.NewComplexFromParts(1.0, 2.0)` | `1+2i` |
...
| `max` | `prim_max_test.go` | 15+ | Mixed types, NaN handling |
| `min` | `prim_min_test.go` | 15+ | Mixed types, NaN handling |
...
| Primitive | File | Test Cases | Notes |
|-----------|------|------------|-------|
| `zero?` | `prim_zero_q_test.go` | 10+ | All numeric types, 0.0 vs -0.0 |
| `positive?` | `prim_positive_q_test.go` | 10+ | Edge: +0.0 |
| `negative?` | `prim_negative_q_test.go` | 10+ | Edge: -0.0 |
| `odd?` | `prim_odd_q_test.go` | 10+ | Integer only, error on float |
| `even?` | `prim_even_q_test.go` | 10+ | Integer only |
| `exact?` | `prim_exact_q_test.go` | 10+ | Integer/Rational=true, Float=false |
| `inexact?` | `prim_inexact_q_test.go` | 10+ | Float=true |
| `exact-integer?` | `prim_exact_integer_q_test.go` | 10+ | |
| `finite?` | `prim_finite_q_test.go` | 10+ | inf/nan=false |
| `infinite?` | `prim_infinite_q_test.go` | 8+ | +inf.0/-inf.0=true |
| `nan?` | `prim_nan_q_test.go` | 8+ | +nan.0=true |
| `integer?` | `prim_integer_q_test.go` | 12+ | 3.0 is integer |
| `rational?` | `prim_rational_q_test.go` | 10+ | inf/nan=false |
| `real?` | `prim_real_q_test.go` | 10+ | Complex with imag=false |
| `=` | `prim_num_eq_test.go` | 20+ | NaN≠NaN, chain |
| `<` | `prim_num_lt_test.go` | 15+ | Chain, mixed types |
| `>` | `prim_num_gt_test.go` | 15+ | |
| `<=` | `prim_num_le_test.go` | 15+ | |
| `>=` | `prim_num_ge_test.go` | 15+ | |
...
|----------------|---------------|
| Numeric binary op | 20 (5 types × 2 args + mixed + edge) |
| Numeric unary op | 10 (5 types + edge) |
| Predicate | 10 (true cases + false cases) |
| Comparison chain | 15 (2-arg + chain + mixed + edge) |
...
4. All numeric types tested: Integer, BigInteger, Float, Rational, Complex
