...
// TestSyntax_ArityErrors verifies that syntax object primitives reject
// wrong argument counts. The VM enforces arity automatically via
// machine_context.go ("expected %d arguments, got %d").
func TestSyntax_ArityErrors(t *testing.T) {
	// Fixed 1 arg: identifier?, syntax->datum, generate-temporaries
	fixedArity1 := []string{
		"identifier?",
		"syntax->datum",
	...
	// Fixed 2 args: datum->syntax, bound-identifier=?, free-identifier=?
	fixedArity2 := []string{
		"datum->syntax",
		"bound-identifier=?",
		"free-identifier=?",
