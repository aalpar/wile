...
- **Library bodies use letrec\* semantics**: R7RS §5.3.2 requires all defined names to be visible to all initializers, enabling forward references. `compileLibraryBegin` and `processFormsWithLetrecSemantics` implement two-pass compilation: pass 1 expands forms via `ExpandBodyWithDefineSyntax` (which compiles `define-syntax` immediately), pass 2 compiles all forms with bindings visible.
- **ExpandBodyWithDefineSyntax unifies define-syntax handling**: R7RS §5.3 requires internal `define-syntax` forms to be processed before expanding subsequent body expressions. `ExpandBodyWithDefineSyntax` in `expander_time_continuation.go` expands forms sequentially, compiling `define-syntax` as encountered. This single function serves lambda bodies, library bodies, and include files.
...
- **with-binding-scope creates placeholder bindings**: R7RS §4.3.2 requires auxiliary syntax like `=>` and `else` to be hygienic. When `(let ((=> #f)) (cond ...))` expands, `let` produces `(with-binding-scope (=>) body)`. The `expandWithBindingScope` function extracts bound identifiers from `(id ...)`, creates a child expand environment with placeholder bindings for each, and expands the body in that environment. This allows the binding checker in macro transformers to detect that `=>` has been locally bound. See `extractIdentifierList()` helper.
- **expandLambdaForm adds lambda scope**: `expandLambdaForm` creates a fresh scope and adds it to both formals and body before expansion. It also creates bindings for lambda parameters in a child expand environment. This supports hygiene by distinguishing identifiers at different binding sites.
- **envBindingChecker uses expander context**: In `OperationSyntaxRulesTransform`, the binding checker uses `mctx.expanderCtx.Env()` (the use-site expand environment) rather than `mctx.env` (macro definition-time environment). This is critical for R7RS §4.3.2 - the pattern matcher needs to check bindings at the macro use site.
