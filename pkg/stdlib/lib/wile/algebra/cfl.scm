...
Keywords: CFL, reachability, worklist, context-sensitive"
  (let* ((node-list (cfl-graph-nodes graph))
         ...
         (node->idx (make-equal-hashtable))
         (nts       (cfl-grammar-nonterminals grammar))
         (m         (length nts))
         (nt->idx   (make-equal-hashtable))
         (prods     (cfl-grammar-productions grammar))
         (unary-rhs (make-equal-hashtable))   ; A-idx -> list of B-idx          (B -> A)
         (bin-rhs1  (make-equal-hashtable))   ; A-idx -> list of (B-idx . C-idx) (B -> A C)
         (bin-rhs2  (make-equal-hashtable))   ; A-idx -> list of (B-idx . C-idx) (B -> C A)
         (R         (make-equal-hashtable))
         (outx      (make-equal-hashtable))
         (inx       (make-equal-hashtable))   ; encoded-pair(A,t) -> list of s-idx
         (work      '()))
