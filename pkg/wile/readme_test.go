...
func TestREADMEGoSnippetsCompile(t *testing.T) {
	// README.md and go.mod live at the module root, two levels above this
	// package (pkg/wile/). Paths are relative to the package directory.
	...
	// The generated go.mod's replace directive must point at the module root
	// (where the real go.mod lives), which is two levels up from pkg/wile/.
...
func assembleREADMESource(blocks []string) string {
	...
	imports := map[string]bool{
		...
		`"fmt"`:                               true,
		`"log"`:                               true,
		`"github.com/aalpar/wile/pkg/wile"`:   true,
		`"github.com/aalpar/wile/pkg/values"`: true,
		`"github.com/aalpar/wile/extensions/files"`: true,
		`"github.com/aalpar/wile/pkg/security"`:     true,
	}
