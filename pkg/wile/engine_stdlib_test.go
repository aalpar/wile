...
// TestEngine_PublicStdLibFS_ResolvesSchemeBase exercises the public
// wile.StdLibFS re-export end-to-end. Unlike the internal stdlib.FS (which is
// prefix-stripped and resolves under the default "." library path), StdLibFS
// retains the "lib/" prefix, so an embedder pairs it with WithLibraryPaths("lib").
//
// This guards the G1 relocation: StdLibFS moved from a root //go:embed to a
// re-export of stdlib.LibFS. The embed shape must be byte-identical for real
// consumers, so this resolves a known library (scheme base) the public way.
func TestEngine_PublicStdLibFS_ResolvesSchemeBase(t *testing.T) {
	c := qt.New(t)
	ctx := context.Background()
