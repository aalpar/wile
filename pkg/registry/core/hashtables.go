...
func addHashtables(r *registry.PrimitiveRegistry) error {
	r.AddPrimitives([]registry.PrimitiveSpec{
		...
		{Name: "hashtable-set!", Mutates: true, ParamCount: 3, Impl: PrimHashtableSet,
			Doc: "Associates KEY with VALUE in HT, replacing any existing entry for KEY.\n\nExamples:\n  (let ((ht (make-equal-hashtable))) (hashtable-set! ht 'a 1) (hashtable-ref ht 'a #f))  => 1", ParamNames: []string{"ht", "key", "value"}, Category: "hashtables",
		...
		{Name: "hashtable-delete!", Mutates: true, ParamCount: 2, Impl: PrimHashtableDelete,
			Doc: "Removes the entry for KEY from HT. Does nothing if KEY is not present.\n\nExamples:\n  (let ((ht (make-equal-hashtable))) (hashtable-set! ht 'a 1) (hashtable-delete! ht 'a) (hashtable-size ht))  => 0", ParamNames: []string{"ht", "key"}, Category: "hashtables",
		...
		{Name: "hashtable-size", ParamCount: 1, Impl: PrimHashtableSize,
			Doc: "Returns the number of key-value pairs in HT.\n\nExamples:\n  (hashtable-size (make-equal-hashtable))  => 0", ParamNames: []string{"ht"}, Category: "hashtables",
		...
		{Name: "hashtable-clear!", Mutates: true, ParamCount: 2, IsVariadic: true, Impl: PrimHashtableClear,
			Doc: "Removes all entries from HT, leaving it empty. The optional size hint K is accepted and IGNORED: R6RS calls it a hint implementations are free to ignore, and the backing store has no capacity knob.\n\nExamples:\n  (let ((ht (make-equal-hashtable))) (hashtable-set! ht 'a 1) (hashtable-clear! ht) (hashtable-size ht))  => 0", ParamNames: []string{"ht", "k"}, Category: "hashtables",
			ParamTypes: []values.TypeConstraint{values.TypeHashtable, values.TypeAny}, ReturnType: values.TypeVoid},
