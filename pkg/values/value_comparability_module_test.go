...
func TestValueImplementorPackages_ListIsComplete(t *testing.T) {
	...
	err := filepath.WalkDir(root, func(path string, d os.DirEntry, err error) error {
		...
		if d.IsDir() {
			// .claude and .worktrees hold git worktrees of this same repository (the
			// EnterWorktree tooling puts them under .claude/worktrees/, plan execution
			// under .worktrees/). Without this skip, a scan run while any worktree
			// exists reports every package in that checkout as "missing from the
			// list", naming <worktree>/pkg/... paths that cannot be added to a list
			// of import paths. The roster is a property of THIS module's source, not
			// of whatever is nested beneath it.
			if d.Name() == "testdata" || d.Name() == ".git" || d.Name() == "dist" || d.Name() == ".claude" || d.Name() == ".worktrees" {
