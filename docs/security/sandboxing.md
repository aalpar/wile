...
What the child namespace does inherit is the authorizer: `Namespace.NewChildNamespace` copies it, so gated operations (`file`, `code`, `env`, `process`, `namespace`, `stream`) stay under the same policy no matter which profile named them. A child given a *stricter* authorizer keeps it: the policy at a gate is `Namespace.EffectiveAuthorizer` — root ∧ child, most-restrictive-wins — resolved from the namespace the code is **executing** in, which for `Engine.EvalIn` is the target rather than the one that registered the primitive. One path does not follow that rule yet: `PrimEnvironment` (the `(wile <profile>)` arm and ordinary import specs alike) and `make-namespace` derive the new namespace from the *registering* namespace (`mc.EnvironmentFrame().Namespace()`), and `checkProfileWidening` reads that namespace's authorizer. Under `Engine.EvalIn` the constructed namespace therefore carries the engine root's policy, not the target's, so a target whose stricter authorizer still permits `code:eval` escapes it with `(eval '(system …) (environment '(wile kitchen-sink)))` on an engine whose root allows `system`. Until that is fixed, do not rely on a child authorizer that grants `code:eval` while the eval extension is reachable. The registry layer is therefore not an authority boundary against Scheme code that holds `environment`; the authorization layer is. If you rely on an extension's *absence* for safety, do not also grant the eval extension, or install an authorizer that covers the operations you care about.
...
Where the target is a path, the check and the subsequent syscall must agree on which file they mean. Containment is not a lexical prefix test: `security.containedInRoot` canonicalizes both the root and the target through `filepath.EvalSymlinks` (resolving as far as the path exists, so a not-yet-created file is still admissible under an existing root), which is what lets a symlinked root such as macOS `/tmp` work and what rejects a symlink staged inside the root that points out of it. It resolves one component at a time without cleaning the path first, and refuses a `..` that backs out of a symlink (the rule `os.Root` itself applies), so every admitted path names the same file whether it is cleaned lexically or resolved by the kernel. On top of that, an authorizer implementing `security.RootConfined` causes both the file primitives (`extensions/files/confined.go`) and the source loader (`resolver/confined.go`) to open through `os.Root`, closing the TOCTOU window between the by-name check and the by-descriptor open. When no root is reported, `resolver.openUnconfined` re-gates the symlink-resolved real path before opening it. `set-current-directory!` has no `os.Root` form; it always enters the symlink-resolved path, re-gated when it differs from the spelling, and its window between that check and `os.Chdir` stays open.
