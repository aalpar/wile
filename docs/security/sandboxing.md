...
One gap remains. The `filepath.Abs` call that precedes symlink resolution cleans `..` lexically, so `<root>/link/..` is judged by its spelling even when `link` points outside the root. The `os.Root` layer keeps the file primitives on that spelled path, but `set-current-directory!` calls `os.Chdir` on the raw string, so under a root-confined profile a symlink already present under the root lets it move the host process's working directory outside the root.
