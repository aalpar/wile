...
| `extensions/eval` | `(wile eval)` | `eval`, `load`, `current-load-path`, `current-load-directory`, `current-load-depth`, `scheme-report-environment`, `null-environment`, `environment`, `expand`, `expand-once`, `compile`, `syntax-local-value/immediate`, `syntax-local-introduce`, `syntax-local-identifier-as-binding` (14 primitives) |
