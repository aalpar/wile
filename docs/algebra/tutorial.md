...
| [`chapters/03-rewriting-basics.scm`](../../examples/algebra/tutorial/chapters/03-rewriting-basics.scm) | `rewrite` -- term protocols, the seven core axiom types with firing and non-firing examples (ch04 adds AC, De Morgan, negation), composed normalizers, manual fixpoint loop |
| [`chapters/04-boolean-simplifier.scm`](../../examples/algebra/tutorial/chapters/04-boolean-simplifier.scm) | `boolean`, `heyting`, `symbolic`, `rewrite` -- `boolean->theory` with 7 axioms (AC normalization + absorption + negation + De Morgan), recursive normalizer + trace inspection, Heyting vs Boolean on `(not (not x))`, `symbolic-boolean-normalize` facade |
