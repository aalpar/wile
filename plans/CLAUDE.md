...
| `2026-05-14-numeric-loss-signals-design.md` | Follow-up to the numeric-registry Phase 3 (Q-i=C3 resolution). Surfaces Go stdlib's discarded loss signals (`big.Accuracy`, `(*big.Rat).Float64()` exact bool) at three layers: (1) Go helpers `ToFloat64Lossless`/`ToFloat64Lossy`/`ToFloat64WithAccuracy` + `werr.ErrLossyConversion` sentinel; (2) FFI tightening — `*BigFloat` accepted when fits losslessly, rejected precisely when lossy (widening of accepted inputs vs. today's blanket rejection); (3) Scheme primitives `(inexact-lossless? n)` predicate + `(inexact-with-accuracy n)` returning `(values inexact accuracy-sym real?)`. Three-phase impl. Five open Qs (Q-1 result type, Q-2 naming, Q-3 library placement, Q-4 FFI opt-in mechanism, Q-5 helpers.ToFloat64 tightening). | **Design draft — implementation deferred until numeric-registry Phase 3 closes.** |
