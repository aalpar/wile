...
| `2026-05-14-numeric-registry-impl.md` | 3-PR implementation plan for the numeric registry. PR 1: new `values/numeric_registry.go` + 7 per-type init registrations + migrate `Simplify`/`ExactnessOf`/`NumberToFloat64`/`NumberToComplex128` to registry. PR 2: migrate `registry/helpers/value_conv.go` (ToFloat64/ToComplex128/ExtractReal); **FFI excluded** per Q-i=C3 — deferred to loss-signals follow-up. PR 3: migrate `extensions/math/{prim_conversion.go, prim_complex.go}` + rewrite ADDING-A-NEW-NUMERIC-TYPE guide. Bench-gate PRs 1 and 2 (≤ 0.5% geomean). Net ≈ −60 LOC + the 10-site update obligation collapses to 1 site. | **Plan locked in; PR 1 not started.** |
