...
| `2026-04-21-matrix-path-d-impl.md` | Matrix Path D implementation plan: 10 phases (scaffold → iterator API → unified accessors → bang-first arithmetic → capability predicate → sparse-error paths → aliasing enforcement → umbrella + docs). D1 resolved to dispatch-table with predicate-cond rep-tag (b-i); D2 resolved to bare `(error ...)` matching existing `matrix.scm` style; D3 resolved to bang-first. OQ6 views additively extensible via `matrix-rep-tag` single extension point. | **Draft** -- all decisions resolved 2026-04-21; 0/10 phases complete |
