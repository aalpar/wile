...
| `2026-05-14-numeric-loss-signals-impl.md` | 3-PR impl plan. PR 1: Go infrastructure — `werr.ErrLossyConversion` + accuracy symbols + per-kind `toFloat64WithAccuracy`/`toComplex128WithAccuracy` methods + values/ exported helpers + `NumericTypeSpec` extension. PR 2: FFI tightening (both Float64 + Complex128 paths) + `WithLossyConversionsAllowed()` engine option + `helpers.ToFloat64` widening per Q-5. PR 3: four Scheme primitives + docstrings + tests over the design's acceptance table. 8-row impl risk register (direction-recovery cost for `*big.Rat`, multi-value-return convention, FFI complex128 additive surprise, engine flag per-instance scope, NaN propagation contract, etc.). Net ≈ +790 LOC across three layers. | **Plan locked-in; PR 1 awaiting numeric-registry Phase 3 merge.** |
