...
| `2026-05-31-interval-dataflow-widening-design.md` | Interval dataflow + Galois connections: wires `fixpoint/widen` into the CFG/MFP worklist solver, adds interval analogs of the sign domain (`sign-binop`), pre-built Galois-connection instances. Extends `(wile algebra dataflow)` + `(wile algebra interval)` + `(wile algebra abstract-domain)`. | **Design draft — not started; `-impl.md` to follow.** |

(Shipped algebra plans — matrix Path D, sparse-dense design, incidence, AC-matching design+impl, group-actions, lattice-birkhoff, combinatorial-graph, wile-goast-algebra-extraction design+impl, polynomial-library, algebra-matching design+impl, **approximate-counting-semirings, graph-worklist-bellman-ford, scc-condensation, graph-cyclic-dispatch, bignum-allocation-reduction (PRs #757–#759), sat-solver design+impl, car-cdr-consolidation, repeated-stanzas-audit (FormParts/Uncons/Spine refactors)** — see Completed Plans → Algebra Libraries / Refactoring.)

### Tech Debt

| File | Contents | Status |
|------|----------|--------|
| `2026-05-07-structural-reduction-roadmap.md` | **Top priority, planning-only.** Cross-package coupling/LOC inventory; recommends Tier A targets (`values/`, `environment/`, `registry/`) for `/structural-reduction` before implementing the `machine/` and `internal/` plans. Tier B/C with appropriate lenses. | **Tier A complete** (env A.2 via PR #730; values A.1 Phases 0–4 + numeric registry + loss signals shipped PRs #747–#756). Tier B/C not started. |
...
| `2026-05-13-values-structural-reduction.md` | `values/` structural reduction (Tier A.1): 9 findings + 4 opportunities. Highest-impact: Port unification (~900 LOC across 9 types collapsible to one struct with capability slots), NumericTypeSpec registry (the 12-item ADDING-A-NEW-NUMERIC-TYPE guide collapses to one record), plus quick wins on `TypeExactInteger` alias, `SchemeTypeName`/`typeNames` duplication, mutex state tightness. Finding 1 (IsVoid convention) recast after design-intent review — original "delete 51 methods, use reflection" framing retracted; recast as additive convention test + `allValueExemplars` roster. | **Tier A.1 complete — Phases 0–4 shipped.** Phase 0 (PR #747), Phase 1 (PR #748), Phase 2 port unification (PR #749), Phase 3 numeric registry (PR #752), Phase 4 `Datum()` deletion + `IsVoid` convention test (commits `d7112b0c`, `e93448c4`). |
| `2026-05-14-port-unification-design.md` | Phase 2 of `values/` SR: collapse 9 concrete port types into a single `*Port` struct with capability slots reached via explicit `AsXxxReader/Writer/...` accessors. Deletes 7 narrow port interfaces (`InputPort`, `OutputPort`, `InputOutputPort`, `BinaryReader`, `BinaryWriter`, `TextualReader`, `TextualWriter`); keeps `Port` marker + `ByteVectorExtractor`. One-PR breaking change; v1.x zero-consumer license. ValueType.Check predicates inspect capability slots, not interface satisfaction. Post-implementation revisions document four mid-flight design refinements (M1–M4): slot-level guarding wrappers, iotest factory in values/, idempotent Close, two-step `get-output-bytevector`. | **Shipped (PR #749, merged).** |
| `2026-05-14-port-unification-impl.md` | 4-phase impl plan for the port unification: Phase 1 introduces `*Port` + 11 accessors + `StringContent` non-breakingly; Phase 2 atomically switches all 10 constructors, deletes 9 concrete types + 7 interfaces, rewrites 6 ValueType.Check predicates, migrates `state.go`/`prim_read_write.go`/`prim_write.go`/`prim_ports.go` (27 sites) + `iotest.go` (slot-injection pattern via new factory) + ~14 test files. Phase 3 cleanup + ADDING guide; Phase 4 PR + dual review. 7-risk register with bench-gate (≤ 0.5% geomean), R7RS conformance, iotest semantic-equivalence. Actual effort ~5–6h vs ~12h estimate. | **Shipped in PR #749.** Implementation outcome + deviations + 7 deferred follow-ups documented at end of file. |
| `2026-05-14-numeric-registry-design.md` | Phase 3 of `values/` SR (Opportunity 3 / Finding 3): single `NumericTypeSpec` record per `NumericKind` collapses the 12-site ADDING-A-NEW-NUMERIC-TYPE guide. Audit retracts two stale parent-plan claims: `wile-goast/goast/mapper.go` does not contain `numberToAST` (the external repo only constructs `values.NewInteger` for AST IDs — no migration required), and `ffi.go:300 schemeToReflectValue` does not exist (the real FFI converters live in `ffi_arg_converters.go:41-96`). Hot path (per-type `[numKinds]func` dispatch tables generated by `makeArithmeticDispatch` and friends) deliberately stays out of the registry — the registry is consulted in *cold* paths only. Q-resolutions baked in: cold-path only (Q-a), drop wile-goast (Q-b implicit), C1 universal `ToFloat64` (Q-c), D1 leave `Eqv` as switch (Q-d), E2 three medium PRs (Q-e). | **Shipped (PR #752).** |
| `2026-05-14-numeric-registry-impl.md` | 3-PR implementation plan for the numeric registry. Q-resolutions baked in: Q-a cold-path only, Q-b drop wile-goast, Q-c→Q-i C3 (5-kind ToFloat64, FFI excluded), Q-d D1 Eqv stays as switch, Q-e E2 three PRs, Q-f drop IsInteger/IsRational fields, Q-g `isAlwaysExact bool`, Q-h unexport spec fields + getter methods. PR 1: new `werr.ErrNumericRegistry` sentinel + `values/numeric_registry.go` (struct + 5 getters + registerNumericSpec + Lookup + ensureNumericRegistryInit) + 7 per-type init registrations + migrate `Simplify`/`ExactnessOf`/`NumberToFloat64`/`NumberToComplex128`. PR 2: migrate `registry/helpers/value_conv.go` only; FFI excluded per Q-i=C3. PR 3: migrate `extensions/math/{prim_conversion.go, prim_complex.go}` (extraction/exactness only — R7RS predicate sites stay per-instance per Q-f) + rewrite ADDING guide. Bench-gate PRs 1 and 2 (≤ 0.5% geomean). Net ≈ −20 LOC. | **Shipped (PR #752).** |
| `2026-05-14-numeric-loss-signals-design.md` | Follow-up to numeric-registry Phase 3 (Q-i=C3 resolution). User-approved and refined 2026-05-14 under "no information loss from Go big to Scheme" mandate. Surfaces Go stdlib's discarded loss signals (`big.Accuracy`, `(*big.Rat).Float64()` exact bool, `(*big.Int).Float64()` accuracy) at three layers with per-component accuracy preserved for complex domain: (1) Go helpers `ToFloat64WithAccuracy` (primary; returns `(float64, big.Accuracy, real bool, error)`) + `ToComplex128WithAccuracy` (per-component) + Lossless/Lossy wrappers + `werr.ErrLossyConversion` sentinel; (2) FFI tightening — Float64 path + new Complex128 path + `WithLossyConversionsAllowed()` engine option + `helpers.ToFloat64` Q-5 widening; (3) four Scheme primitives in `extensions/math/`: `inexact-lossless?` predicate, `inexact-accuracy` (polymorphic — 1 symbol real, 2 symbols complex), `inexact-with-accuracy` (2-value real, 3-value complex), `complex-inexact-with-accuracy` (uniform 3-value). All Q-1…Q-5 resolved: saturate ±Inf, real-triple + per-component complex, extensions/math/, engine opt-in, helpers tightens. Accuracy symbols `'below`/`'exact`/`'above` paraphrase `big.Accuracy` directly. | **Shipped (PRs #753–#755; plans marked complete in #756).** |
| `2026-05-14-numeric-loss-signals-impl.md` | 3-PR impl plan **with detailed code stubs**: complete API contract table for the 5 values/ helpers; concrete `NumericTypeSpec` diff; per-kind helper bodies for all 7 numeric types (including Integer direction-recovery via int64 compare and Rational direction-recovery via round-trip `big.Rat.Cmp`); full `values/conversion.go` skeleton; `WithLossyConversionsAllowed` engine-option code; FFI flag-plumbing diagram (engineConfig → Engine → buildFFISpec → makeArgConverter); concrete `makeArgConverter` extensions for Float64 + new Complex128 cases; `helpers.ToFloat64` Q-5 widening code; concrete `PrimitiveSpec` entries with Doc/Keywords/Category for all four Scheme primitives; full `Prim*` impl stubs using `mc.SetValues` for polymorphic multi-value return; test scaffolding tables. CHANGELOG template + pre-PR-2 audit checklist included. Net ≈ +790 LOC across three layers. References established prior-art primitives `floor/`, `truncate/`, `exact-integer-sqrt` as the multi-value-return template. | **Shipped (PRs #753–#755; #756).** One follow-up open: unify `atan2Operand` with `helpers.ToFloat64` (see TODO.md Tier 6). |
| `2026-05-14-stderr-flush-on-exit.md` | Two-fault chain causes Scheme writes to `(current-error-port)` to be silently dropped on most CLI exit paths: (1) `io` extension is not `registry.Closeable`, so `Engine.Close()` skips port flushing; (2) `cmd/wile/main.go` calls `os.Exit` directly at 9 sites, skipping `defer eng.Close()`. Fix: new `registry.NewCloseableExtension` constructor + `closeIO()` walks the three port parameters; `main()` → `run() int` with single `os.Exit(run())` at top. ~80 LOC, 2-4h, independent of port unification. Open questions: Q-2 Parameter current-value accessor, Q-3 double-close idempotence, Q-4 SIGQUIT graceful shutdown deferred, Q-5 panic path already covered. (Q-1 resolved: new `NewCloseableExtension` constructor.) | **Design locked-in — implementation NOT started (genuinely open).** The `registry.NewCloseableExtension` constructor now exists (landed incidentally via the registry capability-slot refactors), but the two-fault fix itself is unstarted: `io` is not yet wired as `Closeable`, and `cmd/wile/main.go` still calls `os.Exit` directly at 9 sites (no `run() int` wrapper). |
