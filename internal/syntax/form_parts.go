...
//
// On success it returns the elements in source order. On any structural failure
// (not a list, improper list, or arity mismatch) it returns a nil slice and an
// ErrInvalidSyntax-wrapped error naming the form; callers add source context.
func FormParts(form SyntaxValue, name string, minLen, maxLen int) ([]SyntaxValue, error) {
	...
	if !IsSyntaxEmptyList(tail) {
		return nil, werr.WrapForeignErrorf(werr.ErrInvalidSyntax,
			"%s: form must be a proper list", name)
	}
...
// returns the elements unchanged on success.
func checkFormArity(elements []SyntaxValue, name string, minLen, maxLen int) ([]SyntaxValue, error) {
	n := len(elements)
	if minLen == maxLen && maxLen >= 0 {
		if n != minLen {
			return nil, werr.WrapForeignErrorf(werr.ErrInvalidSyntax,
				"%s: expected exactly %d element(s), got %d", name, minLen, n)
		}
		return elements, nil
	}
	if n < minLen {
		return nil, werr.WrapForeignErrorf(werr.ErrInvalidSyntax,
			"%s: expected at least %d element(s), got %d", name, minLen, n)
	}
	if maxLen >= 0 && n > maxLen {
		return nil, werr.WrapForeignErrorf(werr.ErrInvalidSyntax,
			"%s: expected at most %d element(s), got %d", name, maxLen, n)
	}
	return elements, nil
}
