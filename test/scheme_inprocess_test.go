...
func runSchemeFileInProcess(t *testing.T, repoRoot, relPath string) (failures int64, output string) {
	...
	output = captureStdout(t, func() {
		...
		// mutually visible, matching cmd/wile's runFile. WithLoadPath puts the
		// file's directory on the load path so relative include/load resolves.
		runErr = eng.WithLoadPath(absPath, func() error {
			_, evalErr := eng.EvalProgram(ctx, string(source), relPath)
			return evalErr
		})
