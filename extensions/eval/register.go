...
func addPrimitives(r *registry.PrimitiveRegistry) error {
	r.AddPrimitives([]registry.PrimitiveSpec{
		...
		{Name: "syntax-local-introduce", ParamCount: 1, Impl: PrimSyntaxLocalIntroduce,
			Doc: "NOT IMPLEMENTED. Always raises an error; there is no input for which it succeeds.\n\nIntended to flip the introduction scope on a syntax object, to break hygiene deliberately in a macro transformer. It is not wired to the expander: each transformer entry point mints its own intro scope per invocation, so the expander context carries no single scope to flip. Calling it raises not-implemented rather than returning STX unchanged, which is what it used to do — a silent no-op that reported success while flipping nothing.\n\nExamples:\n  (syntax-local-introduce #'x)  => error: not implemented", ParamNames: []string{"stx"}, Category: "eval",
			ParamTypes: []values.TypeConstraint{values.TypeAny}, ReturnType: values.TypeAny},
