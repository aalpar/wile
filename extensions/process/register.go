...
func addPrimitives(r *registry.Registry) error {
	r.AddPrimitives([]registry.PrimitiveSpec{
		...
		},
		{
			...
			ParamCount: 2,
			IsVariadic: true,
			Impl:       PrimProcessSpawn,
			Doc:        "Starts a process with pipes. Returns a process object.",
			ParamNames: []string{"command", "args"},
			Category:   "process",
		},
		{
			Name:       "process-stdout",
			ParamCount: 1,
			Impl:       PrimProcessStdout,
			Doc:        "Returns the stdout input port of a process.",
			ParamNames: []string{"process"},
			Category:   "process",
		},
		{
			Name:       "process-stderr",
			ParamCount: 1,
			Impl:       PrimProcessStderr,
