...
func PrimVectorMap(mc *machine.MachineContext) error {
	...
	mcls, err := helpers.RequireType[machine.Closure](proc, werr.ErrNotAProcedure, "vector-map")
	if err != nil {
		return err
	}
	...
	if len(vectors) == 0 {
		return werr.WrapForeignErrorf(werr.ErrWrongNumberOfArguments, "vector-map: expected at least one vector")
	}
...
func PrimVectorForEach(mc *machine.MachineContext) error {
	...
	mcls, err := helpers.RequireType[machine.Closure](proc, werr.ErrNotAProcedure, "vector-for-each")
	if err != nil {
		return err
	}
	...
	if len(vectors) == 0 {
		return werr.WrapForeignErrorf(werr.ErrWrongNumberOfArguments, "vector-for-each: expected at least one vector")
	}
