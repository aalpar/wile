...
### Architecture
- [~] **Dialect system** [In progress]: forms layer SHIPPED (SP1 per-engine codegen, `WithDialect`, `DefaultDialect`, `R7RSMinimal`, `R5RSStrict`). Phase A primitive-level control SHIPPED (`PrimitiveRemover` capability + `NoMutation` dialect; `feat/dialect-primitive-control`). **Next: Phase B** — expander-level control (make the expander consult `forms.RegistryFor(env)` so removing `import`/`define-library`/`syntax-case` actually disables them → complete strict R5RS). See `plans/2026-07-06-dialect-expander-and-primitive-control-design.local.md`. `plans/ARCHITECTURE.md`
