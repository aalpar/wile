...
| `2026-07-31-go-volume-reduction-discovery.local.md` | Discovery/audit plan. **Phase 0 + Phase 1 complete; Phase 2 (verify) and 3 (rank) open.** Finds redundant/removable/consolidatable Go under a runtime-frozen, compile-time-spendable cost model. Five passes (test→Scheme, metacircular lift, spec-table data, dead/duplicate, lint→wile-goast) |
| `2026-07-31-go-volume-reduction-findings.local.md` | Phase 1 output of the above. 32,873 test LOC (M1, 84% of it in `pkg/registry/core`) + ~12,623 prod LOC across M2/M3/M4/M5 and M6 (runtime moves whose call frequency is exogenous: doc/introspection/REPL, 2,069 LOC). **Nothing verified yet.** Traps recorded: the one prior test→Scheme migration deleted 0 Go lines; the Gabriel gate has no macros so it cannot vouch for M2, and 346 of 371 primitives are never called by it so a green gate is no signal for M6; **Gabriel misranks the primitives it does cover** (`/` is 6.30% there vs 0.030% under the Larceny R7RS suite, `vector-ref` absent vs 10.6%), so use `make bench-extended` for any per-primitive frequency question; the measured Go→Scheme runtime exchange rate is 19–149× per element, contradicting a prior unmeasured "~2×" estimate in `memory/2026-05-03-string-primitives-design.local.md:179`; re-hosting ruleguard in the belief DSL *increases* volume |
