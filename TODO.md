...
| `2026-08-20-pattern-variables-as-bindings-design.local.md`<br>`2026-08-20-use-site-scopes-impl.local.md` | Design proposed, **not started**; the `-impl` twin covers **B only** (4 phases), C gets its own after B merges. Makes a pattern variable an ordinary binding and a template identifier an ordinary reference, so `resolve` decides substitution and `match.TemplateDenotesPatternVariable` + the `binderScopeLog` can be deleted. **Records a correction: this does NOT work alone.** Use-site scopes (Flatt §4.2) are the mechanism that makes resolution refuse the nested-macro capture — measured in Racket, where at module level the use-site scope is the ONLY thing making the pattern and the macro-introduced template incomparable; an earlier expression-position probe read as "use-site scopes are irrelevant" because `local`/`intdef` scopes supplied the same incomparability there. So the sequence is **B (use-site scopes) then C (ordinary bindings)**, and C without B is a behaviour regression that type-checks. **Q1 and Q2 RESOLVED 2026-08-20 by Aaron: mint unconditionally, prune in the expander.** Q1's mint site turned out to be **one** place, not two: `invokeERTransformer` calls `inputForm.UnwrapAll()` on the next line, so **ER macros carry no scopes through and B does not apply to them**. Minting unconditionally sidesteps inventing a definition-context object entirely and holds on all eleven matrix rows; **Q2 prune in the expander** over the existing `extractDefineName`, keeping the use-site registry beside `binderScopeLog` (the compiler sites hold no expander, so a compiler-side registry cannot be read where the pruning happens). Q1's cost is a new `AddScopeToSyntax` walk per macro use because **Wile does not flip** — there is no add-to-input step to piggyback on — and `bench-gabriel` is blind to it. Q2's live risk is the four binder-creation call sites across two functions (`declareDefineBinding`, `predeclareBinding`) which must agree, or one name yields two bindings and forward references break silently. Ships with a mandatory site-count ratchet: every change in this family fails toward deopt, and a total deopt passes the value-assertion suite silently |
