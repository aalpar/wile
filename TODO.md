...
| `2026-08-18-flat-closure-conversion-design.local.md` | **DESIGN 2026-08-18, nothing implemented, nothing measured.** Replaces the escape *proof* with an escape *impossibility*: a flat closure copies its free variables' values instead of capturing `mc.env`, so no closure ever points at a frame. Motivated by the arc's repeated wall — `memory/tail-frame-recycling-unsound` records that a runtime capture bit was implemented and **reverted three times**, because `{Copy, MarkChainShared}` is a chokepoint for continuation capture but `OpMakeClosure` (`machine_context.go:766-777`) is not markable at all. This deletes the second path. **§1 draws the boundary and the doc does NOT cross it:** `call/cc` still captures `mc.env` (`machine_continuation.go:75`), so frames stay heap-reachable and **"therefore tail frames can be released" is explicitly NOT claimed** — that is a separate plan with its own completeness proof (§11). **The estimate is tractable because the analysis substrate already exists:** `WalkBindingRefs` (`validate/walk_binding_refs.go:65`) already reports every reference with a `RefRole` — including `RefSetBangTarget` — and a depth counted in *escaping closure boundaries*, which are exactly the free-variable set (`depth > 0`) and the assignment-conversion set (`RefSetBangTarget` at `depth > 0`); four consumers exercise it. `values.Box` (`values/box.go:22`) is the boxing carrier — **not** `environment/cell.go`, which is the unrelated arithmetic fast path. **§4 is the sizing fork: Design B recommended** (splat the free vector into the apply frame's low slots, so free variables are ordinary depth-0 locals) — **zero new opcodes, zero new `vmState`, no continuation-descriptor row**, reusing `copyForApplyInto`'s existing loop; Design A (closure-indexed, new `OpLoadFree`) is a pure-perf follow-on once the semantics are proven. **Two things it delivers with no further proof:** `bodyCreatesEscapingClosure` dies (§6.1), and **lever B of the OpPushEnv plan loses its gate** (§6.2) — a closure copies values at creation, so `let`-slot reuse is invisible to it, making the merge unconditional across all 3825 sites and recovering the parameter-frame pool leak for free. **That reverses this session's earlier "do lever B first" advice, which predated reading `walk_binding_refs.go`.** **The one silent-breakage risk is §5.2:** `OpSelfTailCall` (`machine_context.go:660`) rebinds slots in place, so a **boxed** parameter needs a fresh `NewBox` per rebind or every closure in the loop shares one cell — nothing in the suite would catch it. §7.1 (transformer closures + `PrimCompile` via `NewClosureWithTemplate`) is unprototyped and is the first thing to try, since the fallback is two closure representations, undoing the deliberate single-representation fold at `machine_closure.go:66-74`. 9 phases; 1–3 are analysis-only and reversible, **phase 4 is the point of no return**. **Gabriel is the wrong gate** (zero `define-syntax`, closures rare) — instrument on the closure-heavy `(adder k)` probe, a site-count ratchet, and a closure-creation micro-benchmark (§7.3 is a real regression direction) |
