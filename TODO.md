...
| `2026-08-07-review-wave1-silent-corruption-design.local.md` | **MOSTLY SHIPPED; merged to master 2026-08-10 at `505c8467`.** On master: §1 (`fix/w1-syntax-case-relocation`), §2 (= IM Phase 0), §3, and **§5 in full** — the base half, the T2 import relocation (`8b8b72f1`) and the `define-syntax` `Imported` clear (`297bb18b`, sequencing §8 item 2). **§6 and `W1-CARVE1` both SHIPPED 2026-08-10** (`ba922024` `fix/w1-6-pattern-literal-definition-pin`; `693ee6f6` `fix/w1-carve1-seal-phase1-registry`), which closes the wave's open list entirely. **§6's mechanism is a definition-TIME pin, not a definition-site env**, and the difference is the whole item: resolving a definition-site *environment* at match time hands back the identical `*Binding` the use site does (a top-level `(define else #f)` made after the macro is visible from the definition frame too — one owner store), so that comparison is vacuous. `match.LiteralPin` carries the resolution captured when the macro was compiled. Storing a resolved `*Binding` — which design §6.3's Caution forbade — is sound in both binding homes after all: globals live in a pointer-stable `[]*Binding`, and a stale local pointer degrades to a forgone match and can never alias a live binding, because Go retains the old array. **The zero pin is deliberately NOT a pin**, so a literal unbound at definition time keeps matching by name (R7RS's both-unbound arm) and `(define-syntax m (syntax-rules (lit) …))` followed by `(define lit 5)` still answers `(LIT 1)` — pinned as a control in `TestPatternLiteralIdentifiedByDefinitionSiteBinding`. The cross-library rows are what discriminate, and they pass. Gate 4's `#` arm is `TestCrossLibraryPatternLiteralNeedsTheDefinitionSiteBinding`. **§4 SHIPPED 2026-08-13** on `fix/w1-s4-single-value-delivery` (`48a52a3c`), and **not as designed** — the plan's two-lever shape is retired, not implemented. §4.3's compile-time single-value predicate does not exist and is **not needed**: the whole family is closed at the point of *delivery* instead of at each drain. `OpPush` had two contracts (deliver one value into one slot; spread a body's 0/1/N to a consumer) and that conflation WAS the defect; the split gives `OpPush` the strict contract and a new `OpPushValues` the spread, the latter reached only from `applyToValuesCode`. With both the optimized and unoptimized paths raising there is no disagreement left for a predicate to close, so **lever 1 is moot and §4.3's "pick one before sizing" fork is closed by retirement**. Two corrections to the plan's own text, both measured: the "cheap conservative" structural whitelist would have **deopted fib** (its `(+ (fib …) (fib …))` args are exactly the compound blocks such a whitelist refuses, and this pass runs *after* `fuseCallForeignCached` so the compound case is its only workload), so "expected bench loss zero" was false for that form; and §4.2's four-row table is **incomplete twice over** — the store rows also leak residue rather than only underflowing (`(define (f) (let ((x (values 1 2))) (list 'a)))` answered `(1 a)`, the extra value drained by an unrelated later call, which no per-drain pre-check can see), and a **fifth** site existed outside the table (`OpPop` at `compile_eval_when.go:234`, fixed in `c192b0fc`). The change also **reverses** the 2026-06-25 continuation-splicing decline; see "Continuation multiple-values follow-ups" below for all four objections re-measured. **All of §4's residuals are now closed too.** The condition is catchable (`werr.ErrWrongNumberOfValues`, a new sentinel — `ErrWrongNumberOfArguments` does not describe `(define x (values))`, which has no arguments), `error-object?` is `#t` and it impersonates neither a file nor a read error; Wave 4 §0's kind clause was struck by its own §9 amendment, so there was no primitive-boundary kind to consume and generic is correct for a VM-domain fault. "Naming the argument position" is answered by **source attribution** rather than an ordinal: `appendPushAt` stamps argument, `let`-init and `define`-value pushes with the SUBEXPRESSION's location, so `(+ 3 (values 1 2))` reports at column 15 and `(let ((a 1) (x (values 1 2))) x)` names the second binding. §4.5's ratchet ships as a **drain-site matrix** (6 sites × {0,1,2} values, `TestSingleValueDelivery_DrainSiteMatrix`) rather than the specified disassembler-vs-VM depth-delta table: that table's premise was "with a per-drain fix, enumeration is the only way to know every drain is covered", and closing the family at the single point of delivery answers it by construction instead. The matrix keeps the plan's actual driver, a program delivering 0, 1 and 2 values into every position, and asserts no row panics. Blockers were all satisfied at ship time: W4-1 landed, and the immutability edge is Phase **2a**, the `Stack` decoupling, which SHIPPED (`pkg/machine/stack.go:32`); Phase 2b changes `values.Vector`, which `Stack` no longer is, so 2b could not have rewritten §4's diff. The sequencing plan's §2.4 read `W4-1 ∧ IM-2` and is corrected to `W4-1`. Wave 1 of the 2026-08-07 review remediation: 6 items, 14 defects (18 with the four, N1–N4, found while resolving Q2), all "correct program, wrong answer, no diagnostic" (`with-syntax`/`syntax-case` frame leak; the three immutable-literal defeats; `=`/`<` not a consistent order; `OpSelfTailCall` and peephole on ≠1 value; `error` missing `InvokesProcedure` plus the unenforced Stable stamp; `syntax-rules` pattern literals). **All three questions resolved 2026-08-07.** Q3: `Stable` is an optimizer hint, not a write barrier; `OpSelfTailCall` gets a runtime guard re-checking its own premise (two conjuncts: not-captured via the `envPooled` analogue, still-immutable via a pinned-binding compare carried on a `NativeTemplate` field). Q1: literal pooling is per-template by design, pinned by a ratchet. Q2: shape **B with the (frame) relocation**, decided 2026-08-09 — the state hangs off the pattern-variable env frame and is resolved by a nearest-marked-ancestor walk from `mc.env`. **The slot RELOCATES; it does not disappear.** (Earlier wording here said "make `(syntax x)` resolve at compile time so the per-context `syntaxCaseState` slot disappears". That is not implementable: the slot holds `bindings`, `matcher` and `input`, and the last two are irreducibly runtime.) **Answering Q2 surfaced FOUR defects the review missed**, all measured: **N1**, `syntax-case` is not reentrant (a nested form destroyed the enclosing form's state, documented at `operation_syntax_case.go:54-59` in a Go comment and nowhere user-facing) — **FIXED**; **N3**, extent asymmetry, `(syntax a)` lexical vs `(syntax (a ...))` dynamic — **FIXED**, and (stack) would have turned it into silent corruption; **N2**, `()` is not a valid whole-clause `syntax-case` pattern (`CompileSyntaxPattern: pattern must be a list`; a `()` **sub**pattern is fine) — **filed, not fixed**, see the Tier-1 row; **N4**, a nested compile scope in a clause body loses the clause's pattern variables — **filed**. N2 is now the only thing still blocking the canonical syntax-list walk (`let loop` plus an inner `syntax-case` with a `()` base case, the shape SRFI-197's reference implementation uses six times). Item 4 must precede the queued `OpSelfTailCall` v2 — **satisfied**, §4 shipped first, so v2 at depth>0 can no longer widen the ≠1-value defect. **All three remaining §8 questions DECIDED 2026-08-08.** Q3 is answered on the *proof* side rather than the consumer, retiring the (a)/(b)/(c) arity-gate fork instead of choosing among it: `Stable` is stamped only where the writer set is closed and known (locals, sealed coordinates — not mutable top-level defines), the three ungated `DefineOwnGlobal` writers refuse a rebind (`Engine.Define`, `Engine.RegisterPrimitive`, `namespace-define!` — the last Scheme-reachable at runtime, since `(interaction-environment)` IS the engine root), imports relocate to **T2** so a top-level define shadows rather than reassigns, and the `Imported` arm of `IsStable()` therefore stays sound. Price accepted: the frame-reclaim payoff on user top-level defines. Q4: two sequenced branches. Q5: price the prompt-frame suspend/resume option first (Wave 3 Q5 owns it), which if it holds removes §4.5's disarmed-measurement caveat entirely. **§2.3's shape D is superseded** — its ordering fix for 16b is replaced by marking the two literal appenders that never mark, see the immutability row |
| `2026-08-07-review-wave2-sandbox-design.local.md` | **PARTLY SHIPPED; merged to master 2026-08-10.** On master: items 11, 7 (its four pins, which enter green by design) and 12; item 8 landed inside the collapsed `PRIM-EVAL` branch (`3d2639cf`). **Decision 7's first half, item 9 and item 10 all SHIPPED 2026-08-10** on `d115f859` (`fix/w2-sandbox-denials-and-resolver`), which closes the wave. The `ErrAccessDenied` pass-through arm is `pkg/machine/foreign_closure.go:97` (`65ed2d8a`); it is **`errors.Is`, not `errors.As`** — the sentinel is static and `CheckWithAuthorizer` wraps every denial with action/resource/target, so both the bare and contextualised forms must match — and it is deliberately NOT folded into `isControlSignal`, which is a positive test for control TRANSFER while a denial is a failure. An already-stamped `*SchemeError` returns unchanged rather than acquiring a second location and trace. Item 10 is `15cc7145`, item 9 `e2dc2451`. **This row's earlier "verified absent at `505c8467`" is what the reconciliation caught: it was true when written and stayed on the page for three days after it stopped being true.** Corrected 2026-08-10: **items 9 and 10 are NOT behind it** — both were verified independently open and independently reachable, and item 10 reproduces by warming the library cache and then calling `SetAuthorizer(DenyAll())`, since `LoadLibrary`'s cache-hit path never reaches `env.FileResolver()`. What IS behind decision 7's first half: Wave 4 item 6, and N-1 slice S1. Top 3 shipped separately on `fix/review-2026-08-07-top3` (§1 records what they did and did not close). 6 items: the `vector-set!` interface tear, the ungated `expand` family, four resolver defects in one package, the library cache surviving an authorizer tightening, `SECURITY.md:96-98`'s missing carve-out, the `profile_containment.go` core under-count. **Q1 resolved 2026-08-07: scope the memory-safety claim to the threads capability**, which drops the tear item from L to S. Two measurements settled it: there is no authorizer gate at either the binding or the value level (zero `AccessRequest` sites on `set!`/`define`/`set-car!`/`vector-set!`), so the threads capability is the only boundary the architecture has; and boxing vector elements closes `vector-set!` while leaving `set-cdr!` open, since a word on the 32-byte cons cell is already rejected on measurement. **All three remaining §8 questions DECIDED 2026-08-08.** Q2: **the intersection**, `security.All(root, child)` at the cache hit (the project's recorded composition rule, shipped as `pkg/security/composite.go:20`) — and the *miss* path's resolver must use the same composite or hit and miss still disagree; item 10 / §5.3(a) is unblocked. Q4: **ship the deny** with an opt-in variant per authorizer, R11's shape (`5b2e826e`) — the deny replaces an already-broken CWD-dependent result, not a working configuration. Q5 **RE-POSED**: measurement falsified the fork's premise — the `load`-catches/`include`-does-not asymmetry is about whether a handler exists yet, not error kind, so option (b) closed nothing. Decided instead, on the maintainer directive that consistency outweighs cost in sandboxing: **denials become unswallowable** (a fifth `applyCallableError` pass-through arm keyed on `security.ErrAccessDenied`, a behaviour change at *every* gate site) **and the error representation is unified** across analysis and execution, per Chibi's `sexp_compile_error`. Adjacent finding filed there, not yet an item: `(eval '(open-input-file "/etc/hosts") (environment '(wile console)))` **succeeds** — the profile namespace does not impose its authorizer on eval'd primitives, which resolve theirs from ctx |
...
| `2026-08-07-review-wave3-broken-subsystems-design.local.md` | **PARTLY SHIPPED; merged to master 2026-08-10.** On master: **item 16** (below), **item 18** (control signals leaking out of `dynamic-wind`, `b3a19261`), **item 13b** (`mc.debugger` propagated into `load`/`eval` sub-contexts, `b9e81df9`) and **item 15** inside `PRIM-EVAL`. **Items 14 and 13a LANDED 2026-08-11** on `integration/wave-2026-08-10` (`b32e7ce8`, `8f3c4c01`): `Engine.Close()` reaps by namespace rather than by closure (the closer seam was per-*registration*, so a shared registry made every engine reap the first engine's children), and the debugger suspends via option C with step-out re-keyed onto `CallDepth()`. **13a's scope is NARROWER than filed, deliberately, and the residue is open:** suspension does **not** reach `load` or `eval`, which still fall back to rendering — accepted 2026-08-11 ("break points don't reach load/eval, will worry about that later"), pinned green-on-both-sides so it is not later mistaken for a gate. Item 13b (`mc.debugger` propagated into the sub-contexts) is the plumbing that makes closing it tractable; the remaining work is the break thunk's own reach. **POSTPONED 2026-08-11 until every wave is complete — it is a separate project, not a wave residual, and must not be scheduled as one.** Sized `M` when accepted; the 2026-08-11 residual audit re-sized it `L` on measuring the actual gap: `NewSubContextWithTemplate` copies the debugger but not the break state (`machine_context_subcontext.go:125-134`), so the sub-context takes the render-only fallback and `RunWithinBoundary`'s break arm (`machine_context.go:1788-1797`) is unreachable. It also consumes the still-open sub-context continuation-truncation surface, which is why it is its own project. Filed under `DEBUGGER.local.md` in the parked index. **Still open, one:** **item 17** (the abandoned-mutex state) — **its PREMISE is wrong, corrected 2026-08-10, and it may need no decision at all.** `MutexState` does not collapse two axes: `MutexAbandoned` is not a HELD state (both wait loops exit on it exactly as on `MutexUnlocked`, `mutex.go:216`/`:223`, and the fast path acquires it without blocking, `:203-207`), so three enum values PLUS the `owner` field faithfully encode all four SRFI-18 answers. The collapse is entirely inside `StateValue` (`mutex.go:144-161`), which returns `'not-owned` from THREE branches and has no `'not-abandoned` symbol at all — so an unlocked, never-abandoned mutex misreports as `'not-owned`. That is a RENDERING bug: one new symbol plus one split branch. Blast radius measured: `Mutex.State()` has ZERO non-test callers, `Mutex.Owner()` exactly one out-of-package caller (`prim_threads.go:603`), and the only Scheme-level pin asserts the WRONG value (`prim_threads_test.go:396-398` requires `(mutex-state (make-mutex))` to be `"not-owned"`). **DONE 2026-08-10 — the renderer fix shipped and nothing survived.** One new symbol plus one split branch; all four SRFI-18 answers now reachable and distinct (`not-abandoned` / `not-owned` / the owner thread / `abandoned`), verified at the Scheme surface. FOUR assertions were pinning the defect and were rewritten — three in `extensions/threads` and `pkg/registry/core` that required a fresh mutex to answer `not-owned`, plus `pkg/values/mutex_test.go:98-112`, whose comment called the sameness *"the load-bearing SRFI-18 collapse the refactor relies on"*, which is the belief this item was filed on. **Item 17's RENDERER half closed as a rendering fix, size S** (`2062fe37`, `dfd8e230`) — but "no maintainer decision required" and "§5.2 is moot" were **both wrong, corrected 2026-08-11**: the renderer fix left **15b** (the terminate-then-lock deadlock) untouched, and §5.2's representation question was live, not moot. **DECIDED 2026-08-11:** owner and abandonment are **independent axes** and the owner **survives** abandonment — `Owner()` in the abandoned state returns the thread that abandoned it, because that retained identity is the debugging information the extra state exists for. The invariant `state == MutexAbandoned ⇒ owner == nil` (`pkg/values/mutex.go:79`) is deleted; heldness reads the flag, never `owner != nil`. One non-test caller of `Mutex.Owner()` (`extensions/threads/prim_threads.go:623`). **15b itself is still open and unsized:** it is a *tracking* gap, not a representation gap — a mutex locked after `AbandonOwnedMutexes` nil'd `ownedMutexes` is in no thread's list, so the flag never reaches it; needs a termination barrier or a lock-time liveness check. A lock-time `Thread.Done()` read is **not** a lock-order inversion (`done` is write-once at `thread.go:139`, only closed after), correcting the constraint recorded when the fork was posed. Two other item-17 residuals stand: delete (not amend) the stale `StateValue` doc comment at `mutex.go:163-171`, and strike the `srfi18-sequencer` Phase 0 pin. **ALL THREE RESIDUALS SHIPPED 2026-08-11** on `fix/w46-file-exists-w317-mutex`, size **S** in total. Representation: `MarkAbandoned` keeps the owner, the `MutexAbandoned ⇒ owner == nil` line is gone, `Owner()`'s contract now says it is not a heldness question. 15b: a **termination barrier**, not a liveness check — `AbandonOwnedMutexes` raises `Thread.mutexesAbandoned` under the same `mutexMu` it already empties the list under, and `TrackMutex` calls `MarkAbandoned` past it, with `mutexMu` released so the existing `Thread.mutexMu → Mutex.mu` order holds. Neither witness the design agonised over is consulted: `Done()` never closes for a Terminate'd-but-unwinding thread and `State()` would need `Thread.mu`, while the flag is set by the very call that creates the gap. **The filed deadlock does not reproduce at HEAD and the gate's stated observation is stale** — terminate-then-lock answers `abandoned` and raises `AbandonedMutexException`; measured for four shapes (tracked lock + terminate, `dynamic-wind` after-thunk on both exit paths, normal exit while holding). A terminated thread's after-thunks do not run at all, and every reachable Scheme path reaches the goroutine's exit defer, which abandons a second time. So the barrier converts an **incidental** guarantee into a structural one; it does not fix an observed hang, and the change is honest about that. The mutex that stays locked forever in `(mutex-lock! m #f #f)` is SRFI-18's `not-owned` case, which specifies no abandonment, and is not 15b. Item 16 SHIPPED 2026-08-09 on `fix/w3-concurrency-cancellation`, atop the `remove-rwmutex-once` removal that cut it from six defects to three. 40: the timed `Mutex.LockContext` timer goroutine now Broadcasts under `cond.L`, closing a lost wakeup reproduced at trial 10596/12000 (3 µs) and 9654/12000 (20 µs), with the `waitOnCondCtx` path as the discriminating control arm. 38: `thread-sleep!` reports `werr.ErrOperationCancelled` + the raw ctx cause instead of a bare `ctx.Err()`, which **restores the producer** its doc comment recorded as absent. 35: `Thread.Join` takes the JOINING thread's ctx, so an untimed `thread-join!` is cancellable — the defect's real reading, not the filed one, whose joinee-terminating form is green without the fix and is kept as a named control. Both 35 and 38 discriminate on the cancellation source (`waitCancelled`) so a parked primitive inside `with-timeout` still runs the handler. **Not started:** the other 5 items — the debugger (never suspends; absent inside `load`/`eval`), vacuous `Engine.Close()`, the load-path stack `PrimLoad` never migrated to (including cross-thread source substitution), the unimplemented abandoned-mutex state, control signals leaking out of `dynamic-wind`. **Q1 resolved 2026-08-07: the debugger suspends, and a breakpoint does it by executing a Scheme thunk** rather than a Go callback. That supersedes an earlier draft proposing a `Run()` return-contract change: the VM blocks inside the thunk the way it blocks inside any nested call, `StepMode`/`StepInto`/`StepOver`/`StepOut` are kept as-is, and `,where`/`,backtrace` work because the context is live at the break instead of finished. 70b (`mc.debugger = p.debugger` in `NewSubContextWithTemplate`) becomes a prerequisite if the thunk runs on a sub-context **All four remaining §8 questions DECIDED 2026-08-08.** Q2: **C — both, closers primary**; wire the two closers through the existing `WithClose` seam (zero call sites today) and leave `Engine` a pure evaluator, with an engine-owned ctx deferred until an embedder asks and gated on writing the ctx-precedence rule first. This also answers `2026-05-14-stderr-flush-on-exit`'s open Q-4 (SIGQUIT/SIGINT handler ownership) — the same question from the other end, do not answer it twice — and **supersedes §2.2's** "recommend the engine context as the primary mechanism". Q5: **price option C first** (prompt-frame suspend/resume on the shipped `RunBodyUnderTimer` shape); A carries the live sub-context continuation-truncation kill-condition and B forces Wave 1 §4.5 to measure disarmed. Q6: **deferred**, and retired outright if C holds. Q7: **dissolution confirmed**, keep `#f` — three obligations follow, one of which is telling the removal plan that its Phase 1 lists `pkg/values/mutex.go` under "Do not touch" while its own verification grep fails on that file |
| `2026-08-07-review-wave4-embedder-contracts-design.local.md` | **PARTLY SHIPPED; merged to master 2026-08-10.** On master: items 2 (+ N-3, duplicate registration made loud), 3, 8, 9 and 11, §0's contract section, and the dead bytecode foreign-call cluster; item 7b landed inside `PRIM-EVAL`. **Item 4 SHIPPED 2026-08-13** (`6b3e1d25`); **5+7 half C SHIPPED 2026-08-14**, closing the design entirely. **Item 1 SHIPPED 2026-08-10** (`76504d70`): `promotedOp` now carries an `identity *PrimitiveIdentity` compared by POINTER, and the name is demoted to diagnostics — so the earlier "take item 1 before Wave 1 §4" ordering is spent. **Item 10 SHIPPED 2026-08-10** (`f5f98f23`), and it did NOT delete the list: `procedureInvokers` stays a literal, now RATCHETED in both directions by `TestProcedureInvokersMatchesInvokesProcedure` against the `InvokesProcedure` annotations, with a `nonPrimitiveProcedureInvokers` map that requires a stated reason per exemption. Reading the `var` alone reads as "still curated" and is the trap this row now names. **Item 6 SHIPPED 2026-08-11.** **5+7 half A SHIPPED 2026-08-13** (this row's own branch): its cross-wave blocker was cluster G, which landed 2026-08-10, so the "landing this first turns `(assq 'x '(()))`'s loud exit 1 into a silent 42" hazard is retired — measured, the handler-with-`call/cc` program now raises. Half A was **half done before it started**: `(environment '())` already answered `caught` (the eval site landed inside the collapsed `PRIM-EVAL` branch), so the residue was the ONE `pkg/registry/helpers/list.go` guard, and `extensions/eval/prim_eval.go` was untouched — the axis-b manifest trap the design serialized three waves around never fired. **The `(environment '())` half being already-fixed left it UNPINNED** — no test asserted it, because it landed under another branch's item rather than its own, so it is pinned now with the `(environment '() '())` asymmetry control the design had used to de-risk the fix in the first place. An incidental fix is exactly the kind that regresses unnoticed. **Item 4 SHIPPED 2026-08-13** (`6b3e1d25`), as §9 Q2 decided: a **direct type assertion, not `errors.As`**. The negative arm is what carries the item — every row of the positive gate still passes under the `errors.As` spelling, so `TestWrappedNativeErrorIsNotForwarded` (`pkg/wile/native_error_passthrough_test.go`) is the only thing that can detect that regression. The `docs/extensions/architecture.md` known-gaps table drops from two rows to one; the surviving row is the `RegisterPrimitive`/`RegisterFunc` panic asymmetry. **Item 6 DECIDED 2026-08-11 and WIDENED:** `file-exists?` answers `#f` for permission errors (authorizer denial *and* OS `EACCES`/`EPERM`, denial staying indistinguishable from absence) and **raises** for every other system error — timeout, `EIO`, `ELOOP`, `ENAMETOOLONG`. That is **two** code edits, not one: `prim_files.go:103` currently collapses every `confinedStat` failure alike, so the OS arm splits three ways. Four shipped assertions pin the outcome it inverts (`engine_sandbox_test.go:624-626`, `:917-918`, `:1044`, `:1100` — the last added by W2-D7a itself) and must be rewritten. A **low-level companion that reports the reason** is anticipated; name, signature and gating undetermined, **design and implementation POSTPONED**, and item 6 must not block on it. **ITEM 6 SHIPPED 2026-08-11** on `fix/w46-file-exists-w317-mutex`, both edits: the authorizer arm answers `#f`, and the OS arm splits three ways through `statAnswersFalse` (`extensions/files/prim_files.go`), which accepts `fs.ErrNotExist`, `fs.ErrPermission` **and** `security.ErrAccessDenied` — the third because `confinedStat` re-gates a resolved symlink target and returns the sentinel from *inside* the stat, so keying on the errno alone would have let that raise. Gates: G5b.1/G5b.2 as a four-row table in `pkg/wile/engine_sandbox_test.go` (`TestAuthorizer_DenialIsIndistinguishableFromAbsence`), G5b.2b must-fail-first on `ENAMETOOLONG` and G5b.2c as a labelled control on `EACCES`, both in `extensions/files/prim_file_exists_test.go`. **FIVE** shipped assertions pinned the inverse, not four: the residual audit missed `pkg/wile/engine_profile_test.go:68` (Console profile, `/etc/passwd`). The two table rows moved out of their tables rather than being deleted — `TestAuthorizer_DenyAllSweep` and `TestDenialIsUnswallowable` each keep a named assertion, because a predicate that answers `#f` cannot be a row in a table asserting `ErrAccessDenied`, and deleting it would have dropped the `file:stat` gate's regression net; **5+7 half A** waits on Wave 5 cluster G, and *landing it first turns `(assq 'x '(())))`'s loud exit 1 into a silent `42`*; **5+7 half C** waits on **W5-G → Phase 6**, corrected 2026-08-10, not on "its own sweep": the sweep RAN, and 11 of its 14 findings close with `fix/wave-prerequisites`. What remains of the prerequisite is ONE guard in `pkg/registry/helpers/list.go` (sweep #1/#2 — one site, not two, since `PrimAssv` delegates to the same `AssocLookup`), owned by Phase 6 and genuinely blocked on W5-G. That guard is what makes the panic population homogeneous, which is Phase 8's own stated precondition. **Half B is RETIRED, not blocked** — the 2026-06-23 reversal was refused and §8 item 1 took shape (C) instead. 9 items, all small and fully independent: the best parallel batch. Promoted-opcode name-string discriminator, `RegisterPrimitive` skipping `Validate`, four instances of the primitive-boundary error model disagreeing with itself, denial not being `file-error?`, `memq` ignoring cancellation, heterogeneous variadic rest under contract enforcement. §0 writes the one boundary contract down. **All three remaining §9 questions DECIDED 2026-08-08.** Q1: **NO — attenuation only.** `Without`/`WithoutCategory` narrow the surface and do not license re-adding a same-named replacement; say so in `primitive_registry.go`'s `Without`/`WithoutCategory` doc comments, which today says neither. Does **not** save the identity plumbing — the fail-open direction still needs the same compare. **Amendment: a duplicate registration becomes loud**, on the `runtime.Scheme.AddKnownTypeWithName` precedent (same name + same implementation idempotent, different implementation fatal; K8s also has no unregister at all, which independently confirms attenuation-by-construction). Go func values are not comparable, so key on `reflect.ValueOf(spec.Impl).Pointer()`. Q2: **make it work** — a primitive may return a `*values.NativeError` and it reaches Scheme intact; **direct type assertion, NOT `errors.As`**, which would also match a `NativeError` under a `werr` wrap and discard the wrap's message. Q4: answered once in Wave 2 Q5 — §0's contract *does* acquire a compile-time half, in the strict direction (denials unswallowable everywhere) plus a unified error representation. **§0's governing sentence, stated by the maintainer 2026-08-08: an authorizer violation does not present itself as an error; authorizer errors stay in the authorizer domain.** **Item 6 RE-SCOPED accordingly** — its original fix (wrap the denial so `file-error?` answers `#t`) is withdrawn as making a sandbox event impersonate an R7RS one; `file-error?` is R7RS-**small** §6.11 from `(scheme base)` and R7RS has no authorizers. The three file edit sites get **no** change (they inherit the pass-through); the item's one code change is **`file-exists?` answering `#f` on a denial** (§6.14 gives it a boolean and no error clause, and `prim_files.go:102` already collapses an OS stat failure to `#f` while the denial three lines above errors — under the pass-through that difference now kills the program instead of being catchable). The five `prim_directory.go` sites keep the denial: commands, non-R7RS, no specified non-error answer. Gate becomes a red/green pair — a denial is not caught by `guard`, an **absent** file still answers `file-error?` `#t`. Ordering **reversed**: Wave 2 first, since item 6 now consumes its pass-through arm. Clearance checked: no Scheme in `pkg/stdlib/lib` guards a file or `eval` operation, so the exposure is embedder code only |
| `2026-08-07-review-wave5-reader-expander-spec-design.local.md` | **PARTLY SHIPPED; merged to master 2026-08-10.** On master: clusters A, B, C, F, H, I and D's **plumbing** half. **W5-E's residual conformance question is RESOLVED 2026-08-11 on `fix/w5e-delete-own-head-exemption`:** `(define define 3) (define define 4)` answered `4`, alone against Petite Chez 10.4.1 and Racket 9.2, which both raise. Wile now raises `application: expected a procedure, got 3`. The fix is a **deletion** — the own-head exemption goes, and with it `formEstablishesBinding`, `definitionBinder`, two parameters of `headDenotesSpecialForm`, and all of `body_entry_env.go` (−227/+97 net across 4 files). The first `(define define 3)` is unaffected in every context (top level and library body via the `Predeclared` arm, lambda/let body via the nil-binding arm), each now ratcheted separately since nothing pinned the correspondence. The alternative — refuse to bind a keyword name — was **rejected**: it would refuse the *first* definition, which R7RS §5.3.1 p.26 licenses and both peers accept, it needs a name-keyed check the binding-identity invariant forbids, and it has no authoritative keyword set (validate registry 38 names vs `compileTimeBindingSpecs` 41, symmetric difference 7, and `RegistryFor(env)` is per-namespace so the bindable set would become engine configuration). **Three shapes still diverge and are deliberately left:** a top-level shadow reaching into a lambda body, and both macro-mediated forms — all turn on `referenceReachesBinderDirectly`'s hygiene guard, whose relaxation is its own arc. **Cluster E LANDED 2026-08-11** on `integration/wave-2026-08-10` (merge `abf0ddbc`) — it deleted the **validator's CALL to** `HasLocalVariableBinding` rather than patching the method (which keeps its two expander callers), inverted the dispatch to `GetBinding` with scopes, and absorbed the identity-keyed dispatch of sequencing §8 item 4. Two R7RS conformance defects surfaced *by* the arm were fixed in it: §4.2.3's top-level `begin` splice (`397d6af8`) and §5.3.2's whole-body binder region at a definition's own head (`fda10178`); see `plans/2026-08-11-toplevel-begin-splice-conformance.local.md` for the four-way Chez/Racket measurements and the four questions it leaves open. **Cluster G and cluster D's semantics half (9C) both SHIPPED 2026-08-10** (`7799fb24` `fix/w5g-noncontinuable-escalator-arm`; 9C rode in on the §6 pin branch), which closes the wave. Cluster G's fix is **per-arm revival decided at the `(k v) site`**, not the driver's global `resumeGeneration` window: both a `guard` miss-path re-raise and an ordinary `call/cc` escape inside the handler capture a segment CONTAINING the finalizer frame, so segment membership alone is vacuous — they differ in whether the live chain at the `(k v)` site had already discarded the frame. Judging at reinstatement instead reads the frame as absent for a jump that never left it, which swallows the secondary again; `pendingEscalatorRevivals` is therefore called by the invoker, not by `ReinstallSegment`. Verified at HEAD for all three of §7's forms — the `(error "boom")` witness, the helper-procedure trap form, and the `(car 5)` widening — each now raising `exception handler returned from non-continuable exception`, exit 1. 9C is the `LiteralDefs` field on `SyntaxCaseClause` plus the pass at `operation_syntax_case.go:231`; it has to ride on the clause because that path builds its matcher at RUNTIME, in the use site's environment, the one place that cannot recompute a definition-site resolution. 9 clusters, 36 defects (the design states 32 / 34 / 36 in three different places; **36 is correct**, crossplan §2.6, and this row's earlier 32 was the number Wave 5's Phase 5 was scheduled to correct). Two design ideas carry most of it: the number scanner must be speculative (6 defects collapse), and a pattern literal is identified by binding not spelling (shares Wave 1 §6's representation). Plus parser, validation, quasiquote, the resume-generation counter, six numeric defects, two nondeterminism defects. **Q2 resolved 2026-08-07: special forms can be shadowed by `define`, `let`, and any other binding form** — a special form is a fallback, not an authority. That sharpens cluster E: `validate/validate.go:96-134` is wrong two ways, its shadow check is local-only (so a top-level `define` or an imported rename is invisible, which is why `(rename put! set!)` cannot work) and its registry lookup is keyed on `symVal.Key`, a name string, in violation of the Binding Identity invariant. Fix is extend-the-env **plus** invert the dispatch to `GetBinding` with scopes; special forms already have bindings at the ambient T3 tier that a user `define` at T1 outranks, so the environment has been giving the right answer and the validator has been refusing to ask it. Gate on a `bench-gabriel` A/B (a global probe per form head) and sweep the stdlib for names that would newly shadow |
| `2026-08-07-review-wave6-cli-stdlib-docs-design.local.md` | **PARTLY SHIPPED; merged to master 2026-08-10.** On master: Phase 0 (§5.1, `test/run-all.sh` can now see a failing Scheme suite — the prerequisite under *every* Scheme gate in *every* wave), clusters A, B and C (§1, §2, §3), and §8 mechanisms (0), (4) and (2) (`b767a83f`, `make indexlint` + `docs_counts_test.go`). **§4 and mechanism (1)'s count half LANDED 2026-08-11** with `docs/w6-drift-and-premise-ratchets` (merge `996e9b12`) on `integration/wave-2026-08-10`: **§4**'s five documentation drifts went in as one pass, unblocked by §8.2's prose-count checker; **mechanism (1)** is **SPLIT, and its count half landed in the same range** — the core-name premise is no longer hand-typed: `pkg/internal/bootstrap/profile_containment_test.go` derives it from `testdata/axis-b-manifest.scm`'s symbol column and asserts SET equality against `extensionPrimitiveNames(nil)`, reporting both directions of the difference (`c.Check`, so a rename prints as a rename). Its neighbouring comment had already rotted (it claimed 467 manifest rows against 455), which is the class the mechanism exists for. **Mechanism (1) is CLOSED — its static half was already on master when the 2026-08-13 sweep filed it as open.** Wave 5 cluster E owns and shipped the `CompileSymbol` reachability ratchet (`4fc4020f`, hardened in `2c6dbffc`): `pkg/machine/compilation/compile_symbol_reachability_test.go`. Both §8.4 gate instances were re-verified by mutation on 2026-08-13, not by reading. What shipped is **one ratchet per premise, in the package that owns the premise**, not the `cmd/wile/` premise TABLE the Delivery section priced; the gate does not require the table and the per-package shape is stronger, so this is a deliberate deviation, not a residue. Mechanism (3) stays a tombstone and schedules nothing. 17 defects (completer crash on `éé<TAB>`, go-flags stripping a leading quote from `-e`, shebang, MCP `"timeout":0`, raw ESC bytes in `(chibi term ansi)`, a `(chibi test)` that cannot fail, `validate-ring` accepting a non-ring, five export-list deviations, five doc drifts) **plus §8, the cross-cutting process ratchet** the review's §7.7 argues for: §8's evidence is thirteen rows covering twenty-seven sites — the review's nine rows / sixteen sites, of which four match the survived-its-own-remediation framing, plus three from the 2026-08-08 `plans/` sweep and one in the index itself — and 3 more are guarded by a test structurally unable to see them (`TestRunEval` is not one of them: `cmd/wile/main_test.go`'s `runCLI` re-execs the test binary through `TestMain`, so go-flags parsing is exercised end to end). §8 is worth more than any single fix in the six waves and must land before the doc-count edits. **All three remaining §7 questions DECIDED 2026-08-08.** Q2a: **B — keep `call/cc` in `(scheme r5rs)` and document it.** The 2026-07-14 API-stability policy (`TODO.md`'s own checked item, `cc3c48bb`) is unconditional and admits no conformance carve-out even where both R7RS authorities exclude the export; add `(scheme r5rs)` supersets rows for `call/cc` **and** `unquote`/`unquote-splicing` together (the table has none today), keeping the library at 225 exports. `call/cc` is not one of the five pinned superset rows, so dropping it would have failed no test — the silent break the policy exists to prevent. Q4: **C — drop §8.3 mechanism (3)**, reallocating to (4); its executable residue (a *suite* named for a property must state which gates it does not cover) moves into (0) as a gate clause, which changes what §8.4 gates. Q6: **deferred** behind Wave 3 Q5's option C, under which nothing needs writing into §1.4's invariant |
| `2026-08-07-review-wave1-silent-corruption-impl.local.md` | **Phases 1–5 SHIPPED and merged 2026-08-10; Phases 5b, 6 and 7 open.** **ALL PHASES SHIPPED.** Phases 5b (`W1-CARVE1`) and 6 (§6) landed 2026-08-10; Phase 7 (§4) landed 2026-08-13. Nothing in this plan is open. **The checkboxes in this file were never ticked, including on the shipped phases** — read the status line here and the sequencing doc's §2.4, not the boxes. `-impl` twin of the Wave 1 design, written 2026-08-09. Phases in the sequencing doc's stage order: §3, §2 (= the immutability plan's Phase 0), §1, then §5, then §6 and §4. Q2's relocation is **decided: (frame)**, resolved by a nearest-marked-ancestor walk from `mc.env` rather than a compile-time depth immediate — and it closes a third unfiled defect, **N3**, that `(syntax a)` has lexical extent while `(syntax (a ...))` has dynamic extent. Q6 is **GREEN** (literal-pool ratchet: 0 near-misses across every suite). Q7 is **BUILT**, not a fallback — `pkg/wile/error_selftail_capture_gate_test.go`. §5 carries the T2 import relocation, sized S-code/M-decisions and **not separable** from §5's branch |
| `2026-08-07-review-wave2-sandbox-impl.local.md` | **Phases 1–4 SHIPPED and merged 2026-08-10; Phases 5, 6 and 7 open.** **ALL PHASES SHIPPED 2026-08-10** on `d115f859`: Phase 5 (denials unswallowable), plus 6 and 7, which were independently reachable rather than behind it. Wave 4's Phase 5 consumed Phase 5 and has since landed too. Phase 5's own line citations are STALE: `b3a19261` moved the four `errors.As` arms into `isControlSignal`, so the fifth arm belongs in `applyCallableError` beside the `isControlSignal` CALL, not inside it — that function's doc comment forbids a fifth chain, and a denial is not control flow. Checkboxes unticked throughout, shipped phases included. `-impl` twin, written 2026-08-09, 7 phases / 96 checkboxes. Adds a phase the design's §9 table predates: decision 7's first half, the fifth `applyCallableError` pass-through arm, sequenced *before* items 9 and 10 because their gates assert non-absorption. Item 8 is not a phase here — it is a labelled contribution to the collapsed `PRIM-EVAL` branch. Item 7's four pins are recorded honestly as **ratchets, not must-fail-first gates**: item 7 restates an accepted deviation, so all four enter green, and pin 3d is a canary calibrated to fail when the immutability plan's Phase 2 lands |
| `2026-08-07-review-wave3-broken-subsystems-impl.local.md` | **Phases 1, 2 and 4 SHIPPED and merged 2026-08-10 (plus item 15 via `PRIM-EVAL`); Phases 5 and 6 landed 2026-08-11 on `integration/wave-2026-08-10`; Phase 3 is the only one open.** Phase 6 shipped 13a's core only — suspension inside `load`/`eval` is **POSTPONED until every wave is complete** and re-filed as a separate project (`DEBUGGER.local.md`), not a wave residual; see the design row. Phase 6 therefore counts as CLOSED for wave purposes. **Phase 3 (item 17) CLOSED 2026-08-11** — renderer (`2062fe37`, `dfd8e230`), then representation + 15b + the two doc residuals on `fix/w46-file-exists-w317-mutex`. The maintainer question §5.2 posed was answered, not moot: owner and abandonment are independent axes and the owner survives abandonment. 15b closed as a **termination barrier** in `Thread.TrackMutex`; see the design row for why the filed deadlock does not reproduce and what the barrier therefore buys. Checkboxes unticked throughout, shipped phases included. `-impl` twin, written 2026-08-09. Item 16 is **three** defects (35, 38, 40), sized M, gated on the rwmutex removal. Item 13 uses **option C**, priced 2026-08-09, which forces a user-visible behaviour change: step-out must be re-keyed onto `CallDepth()` because `SliceContinuationAt` deep-copies frames and the pointer comparison at `debugger.go:176` degenerates into step-into after the first resume. Defect 35's filed gate was **green at HEAD** under the joinee reading and is re-specified on the joining-thread reading, which hangs. Item 17 is unsized and left as a phase with a blocked head |
| `2026-08-07-review-wave4-embedder-contracts-impl.local.md` | **Phases 1–3 SHIPPED and merged 2026-08-10; Phases 4 and 7 shipped in the 2026-08-11 batch; Phase 5b (item 6) SHIPPED 2026-08-11 on `fix/w46-file-exists-w317-mutex`.** Open: none. Phase 5's item-4 half shipped 2026-08-13; **Phase 8's half C shipped 2026-08-14** (`fix/w4-half-c-ffi-recover`) — see the Phase 8 row in that file for the four corrections it makes to its own text, chiefly that both of its stated prerequisites had already shipped and that G8.4 cannot be must-fail-first. **Phase 6 (5+7 half A) SHIPPED 2026-08-13, gate complete** — its blocker, Wave 5 cluster G, landed 2026-08-10, and only the `pkg/registry/helpers/list.go` guard remained. §5.3's gate is all three parts, in `pkg/wile/panic_channel_gate_test.go`: both domain cases (`assq` and `(environment '())`) plus the **negative arm**. The arm registers ONE panicking body by BOTH routes in one engine — `RegisterPrimitive` takes the `Impl` as given and its panic escapes `guard` to the embedder; `RegisterFunc` installs `ffiSpec.makeWrapper`, whose defer turns the panic into a returned error, which `guard` catches. Same fault, opposite answers, so the pair is non-vacuous by construction and a reinstated blanket recover would flip the first row. That asymmetry IS half B's subject, and half B was refused — the test records which way each route falls rather than asserting they agree. Checkboxes unticked throughout, shipped phases included. `-impl` twin, written 2026-08-09, 8 phases. **Phase 8's blocked status is HISTORY, not current:** the reversal of the 2026-06-23 "panics stay within VM boundary" directive was **refused**, and the phase was re-shaped as half C (tighten the FFI recover) and is schedulable; checkboxes are filled for both the granted and refused branches. New intra-wave conflict found and handled: items 3 and 11 both edit `OperationPushWind.Apply`, and after item 11 widens the operand check to `values.Callable`, item 3's gate is **structurally destroyed** because `compileDynamicWind` applies `before` before emitting `PushWind`. Also found: the rwmutex removal's Phase 2 misses two files that trip its own verification grep |
| `2026-08-07-review-wave5-reader-expander-spec-impl.local.md` | **Phases 1–4, 6, 7 and Phase 9's plumbing half SHIPPED and merged 2026-08-10; Phase 5 landed 2026-08-11 on `integration/wave-2026-08-10`; Phases 8 and 9C open.** **ALL PHASES SHIPPED**: Phase 8 (cluster G) 2026-08-10, and 9C with the Wave 1 §6 pin the same day. Checkboxes unticked throughout, shipped phases included. `-impl` twin, written 2026-08-09, nine cluster phases / 188 checkboxes. **Six gate homes could not fail as naively sited** and are re-specified — cluster I needs forked processes (map disorder is per-process) and keys on table *kind*, not entry count; cluster D 2.2.14's only driving test survives the fix; cluster E's guard test needs an engine built **with** library paths; cluster C 2.2.10 is unreachable from Scheme. Cluster D carries AU.1's three deletion constraints as checkboxes. Two of the design's own claims corrected: the flat-binding `-impl` **does** have an index row, and `continuation-test.scm` has no sibling — the other file never matches `run-all.sh`'s glob |
| `2026-08-07-review-wave6-cli-stdlib-docs-impl.local.md` | **Phases 0–6 SHIPPED and merged 2026-08-10; Phase 7 landed 2026-08-11 on `integration/wave-2026-08-10`; Phase 8 open.** Phase 8 runs last — its three prerequisites are all on that branch now (see the design row above). Checkboxes unticked throughout, shipped phases included. `-impl` twin, written 2026-08-09. Mechanisms (0), (4) and (2) of §8 were **built 2026-08-09** and merged (see the process-ratchet row below); (1) is scheduled last, after Wave 5 cluster E, Wave 4 item 10 and Wave 2 item 12; (3) is a tombstone |
...
| Plan | Status |
|---|---|
| `2026-08-08-review-wave-crossplan-audit.local.md` | **Audit, complete; §5 CLOSED 2026-08-08.** Cross-plan pass over the six wave plans plus the immutability design: 545 claims verified, 99 findings, and it is the authority on how the seven plans order against each other (§4) and on what each plan had factually wrong (§3). **§5's ledger of 15 maintainer decisions is now answered in full**, every verdict written into its owning plan; the verdict and the standing recommendation diverge on five rows, and in four of those the divergence came from a premise falsified during the sitting rather than from taste. Three findings the sitting produced that were not on the ledger, each larger than the question that surfaced it: **two literal appenders never mark** (`CompileSelfEvaluating`, `compileQuasiquoteDatum`'s constant path), so `(vector-set! #(1 2 3) 0 9)` and ``(vector-set! `#(1 2 3) 0 9)`` succeed at `003b3353` and literal immutability is **order-dependent within a compilation unit**; the `load`/`include` guard asymmetry is **not** an error-kind difference but a question of whether a handler exists yet, which falsifies the option both Wave 2 Q5 and Wave 4 Q4 were built around; and `native_template.go`'s dedup-rebuild cluster (six functions) is **production-dead**, its only non-test caller being its own recursion. §6's five work items and §7's follow-ups outside the plans are untouched and still open |
| `2026-08-09-unified-error-representation-design.local.md` | **Design, not started.** The `N-1` work decision 7's second half implies, which belonged to no plan. **The filed "~140 sites / 17 files" prices the wrong axis:** that is provenance *attachment* (really 220 sites / 27 files) and unification leaves all of it alone; the *representation* axis is 12 construction call sites in 3 files behind one production constructor. Verdict **M**, sliced S0–S3, and **no wave item consumes it** — Wave 2's denial sites and Wave 4 item 6 consume decision 7's *first* half, which shares no code. So S0 (provenance repair) and S1 move forward into stage 1; S2/S3 go last because nothing waits on them. Surfaces three CONFIRMED provenance defects (VERDICTS 92/93/94) that belong to no wave and no plan, and one measured hazard: shipping unification over 92 promotes a *wrong* location into a Scheme-visible accessor |
...
| `2026-08-09-wave-impl-sequencing.local.md` | Derived schedule for the seven 2026-08-07 wave/immutability plans plus the rwmutex removal, against HEAD `003b3353`. Not a new argument: every edge is stated in at least one plan, and disagreements between plans are called out and resolved. **Live status lives here, and §2.4 is the residual schedule** (updated 2026-08-10 after the merge): 26 branches shipped and all merged to master at `505c8467`, original stages 0 and 1 empty, terminal nodes unchanged (`W6-8.1` and `W1-§4`), longest surviving chain 3 deep. Ten items are READY with no unmet blocker; five of those touch disjoint files and share no serialized artifact — Wave 2 Phase 5, Wave 5 clusters E and G, Wave 1 §6, Wave 6 §4. **The `-impl` files' checkboxes were never ticked, on shipped phases included**, so §2.4 and the plan rows above are the status of record and the boxes are not. **§2.4a, added 2026-08-10, is the correction block: every open item was re-verified in the code and SIX of §2.4's blocker claims were wrong** — five shorten the schedule (W1 §4 waits on W4-1 alone; W2 items 9 and 10 are not behind decision 7; W4 half C waits on W5-G → Phase 6; N-1 S3 has one blocker; sweep #1/#2 are one site) and one lengthens it (N-1 S1 is not "blocked on nothing"). It also carries the two retaken measurements §8 asked for: W1-CARVE1's collision surface is still empty (146 / 60 / ∩0) and W6 §8.1's 208 is measurable today. Read §2.4a before scheduling from §2.4 |
...
- [ ] **Expand-phase registry primitives are NOT sealed** [Correctness, S code / M test rewrite,
  ...
  **Sized and decided**, because the asymmetry also makes the `Stable` stamp
  ...
  the stamp to phase 0 instead) in `plans/2026-08-07-review-wave1-silent-corruption-impl.local.md`
  Phase 5b and `plans/2026-08-09-wave-impl-sequencing.local.md` §8 item 3b.
...
- [ ] **A cell-yielding form of the canonical list walker** [Refactor, S–M, **deferred on YAGNI —
  ...
  > The substance above is rebuilt from `plans/2026-08-07-review-wave4-embedder-contracts-impl.local.md`
