...
- [ ] **Documentation system** [Tooling, High, Partially Complete]: REPL-accessible documentation for all Scheme bindings. Completed: `procedure-documentation` primitive (PR #579, #581) for MachineClosure, ForeignClosure, CaseLambdaClosure; all 397 `PrimitiveSpec.Doc` fields populated; ~243 stdlib library procedure docstrings (PR #582); `,doc` REPL command via `DocProvider`. Remaining gaps: (1) bootstrap procedures lack docstrings, (2) `,doc` and `procedure-documentation` are separate paths, (3) special forms/macros have no doc mechanism, (4) no `apropos`/search, (5) no library-level or cross-reference docs.
  ...
  - [x] **Unify `,doc` with `procedure-documentation`** [Unification, Done]: `formatBindingDoc` tries `callableDoc()` first (same extraction logic as `procedure-documentation`), then falls back to `bnd.Doc()` for compile-time-only bindings (special forms, auxiliary syntax). PR #585.
  - [x] **Docstring validation in validator** [Separation of concerns, Done]: Moved docstring detection from compiler to `internal/validate/` — `ValidatedLambda` and `ValidatedDefine` now carry doc metadata. The validator recognizes syntax, the compiler just reads a field. Prerequisite for Scribble-style `@` notation. Branch `refactor/docstring-validation`.
  ...
  - [x] **ApplyDocs unresolved name diagnostics** [Robustness, Done]: `TestDocEntryNamesResolve` in `registry/core/specialforms_test.go` boots a full environment and verifies every documented name (BindingSpecs with Doc + DocEntries) resolves to a binding in at least one phase. Catches typos at `make test` time with no runtime API changes.
  - [x] **`apropos` / documentation search** [Usability, Done]: REPL command `,apropos pattern` (alias `,a`) and Scheme primitive `(apropos "pattern")` search names, docstrings, and categories (case-insensitive substring). REPL output shows `name [category] doc`. Primitive returns sorted list of symbols. Branch `refactor/unify-doc-procedure-documentation`.
  - [x] **Library-level documentation** [Infrastructure, Done]: Non-standard `(description <string>)` clause in `define-library`. Parsed by `processLibraryDescription`, stored on `CompiledLibrary.Description`. REPL: `,doc (scheme base)` shows description + exports. Scheme: `(library-description '(scheme base))`. All 34 stdlib `.sld` files documented. Branch `feat/library-level-docs`.
  - [x] **Cross-referencing and topic browsing** [Usability, Done]: REPL commands `,topics` (list categories with counts) and `,topic <category>` (list entries). Scheme primitives `(doc-topics)` (sorted list of category strings) and `(doc-topic "category")` (sorted list of symbols). "See also" cross-references deferred (no data source). Branch `refactor/unify-doc-procedure-documentation`.
  - [ ] **Syntax compiler docs** [Content, Gap]: 11 compile-time bindings registered via `RegisterSyntaxCompilers` (`syntax_compilers_registry.go`) have no documentation: `syntax`, `syntax-case`, `meta`, `define-library`, `library`, `import`, `export`, `quasisyntax`, `unsyntax`, `unsyntax-splicing`, `with-syntax`. Add to `compileTimeBindingSpecs` in `specialforms.go`.
