...
### Architecture
- [~] **Dialect system** [In progress]: forms layer SHIPPED (SP1 per-engine codegen, `WithDialect`, `DefaultDialect`, `R7RSMinimal`, `R5RSStrict`). Phase A primitive-level control SHIPPED (`PrimitiveRemover` + `BootstrapProcedureRewriter` capabilities; `NoMutation` removes ALL 13 mutators genuinely — mutating `vector-map`/`string-map` swapped for a mutation-free bootstrap fragment; inline-HOF optimizer gated on `requires` so removal deopts cleanly). Phase B R6RS-macro control SHIPPED (R5RSStrict removes `syntax-case`/`with-syntax`/… via `fr.Remove` — they carry compiler FormSpecs, so no expander gate is needed; the design's expander-gate premise was falsified). Branch `feat/dialect-primitive-control`. **Remaining (deferred):** disabling `import`/`define-library` for a dialect — the only genuine use of an expander-registry gate, but `import` is bootstrap-coupled and needs a bootstrap-vs-user phase boundary (D-B2). Tested as inversion targets (`TestR5RSStrict_ImportNotDisabled_ExpanderCeiling`, `TestNoMutation_ImportReexposes_DocumentsBoundary`). See `plans/2026-07-06-dialect-expander-and-primitive-control-design.local.md`. `plans/ARCHITECTURE.md`
