...
- [ ] **Audit PrimitiveSpec `ReturnType` and `ParamTypes` annotations** [High, L] — **mostly complete; 5.D/5.E remain**: Phase 1 axis-A (docs ↔ ReturnType) clean. Phase 3 axis-B (ReturnType ↔ impl, 5 tightened in PR #675). Phase 4 axis-C R7RS sweep clean. **Phase 5 ParamTypes audit: 5.A (manifest, PR #678), 5.B (analyzer + inventory, PR #679), 5.C (sidecar, `plans/2026-04-20-paramtypes-annotation-bugs.md`) complete.** 5.C finding: declared-too-narrow bucket empty (3 FPs confirmed); declared-too-wide dominated by TypeConstraint-vocabulary gaps (~85 entries — same deficit as axis-B Category C, viewed param-side), not mechanical errors. 5.D scope reduced to 1 opportunistic partial tightening; primary follow-up is analyzer refinement. 5.E (axis-C R7RS sweep over ParamTypes) pending. Original scoping rationale below:
