...
- [x] **Sub-context winding stack inheritance hazard** [High, Correctness, Fixed]: `NewSubContext()` now inherits the parent's winding stack automatically. `NewSubContextWithWinding(ws)` added for the two sites that need a truncated stack (unwind and exception cleanup). `machine/machine_context_subcontext.go`.
