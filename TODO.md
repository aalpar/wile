...
- [x] **Port type Value method boilerplate** [Medium, S]: `SchemeString` consolidated into `portBase` via `kind`/`datum` fields and `initPort` helper. `IsVoid` stays per-type (nil-receiver on value-embedded struct panics before reaching promoted method). `EqualTo` stays per-type (requires concrete type assertion).
