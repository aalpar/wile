...
**The twelve 2026-08-07 wave plans left this index on 2026-08-14**, when Wave 4's
Phase 8 half C closed the last open item across all six waves. They are in
`memory/`, indexed by `memory/INDEX-reviews-and-remediation.local.md`, and each
file carries its final Plan Index row verbatim in an ARCHIVED banner — so nothing
below was deleted, it moved. Two things the arc left behind are still live and
are filed on their own merits, not as wave residuals: the debugger's reach into
`load`/`eval` (`DEBUGGER.local.md`, parked index) and the low-level file
predicate under Tier 2. The wave arc's own schedule and cross-plan audit
(`2026-08-09-wave-impl-sequencing`, `2026-08-08-review-wave-crossplan-audit`)
remain in `plans/` and are archivable.
