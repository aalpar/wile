...
| `2026-08-03-r6rs-hashtables-design.local.md`<br>`2026-08-03-r6rs-hashtables-impl.local.md` | Impl plan written 2026-08-03, 14 tasks, **not started**. All six Qs resolved (Q1 defer Phase 4, Q4 leaf-gate + same-kind). Verifying the design against HEAD found one defect: §4.3's ordinal-visited-set `EqualHash` breaks the `Equal ⟹ same hash` contract — `(1 2 . A)` and `(1 2 1 2 . B)` are `equal?` with 2 vs 4 distinct nodes; replaced by a budgeted unfolding walk. Four smaller corrections: `helpers.Eqv` must be lifted into `pkg/values` first; `hashtable-{equivalence,hash}-function` are **Go** (this correction was withdrawn — see below); `hashtable-update!` belongs in the dialect-swappable bootstrap fragment, not `bootstrap_procedures.scm`; Go-test call-site counts were stale. Closes the "SRFI-125 or keep custom" fork by choosing R6RS. **Revised 2026-08-03 on Chibi/Guile evidence:** both encode the same table-owned discriminant (Chibi a fixnum tag in the record's hash/eq slot, `lib/srfi/69/interface.scm:19-27`; Guile an `'eq \| 'eqv \| 'custom` symbol), and the tag **replaces** the procedure rather than sitting beside it — so recognition costs nothing per lookup. Two changes follow. **Q6 now uses pointer identity** against the namespace's sealed base (`registerPhasePrimitive` builds one closure per spec per namespace, so a pointer compare *is* `eq?`), superseding the `.Name()` string compare, which **fails open** when an embedder registers its own `equal-hash`; cost is a wash, the argument is robustness. **Q1-B (Scheme-record custom table) is rejected** by maintainer directive — neither reference implementation has a second representation, so Phase 4, when it lands, widens the `kind` slot to hold a `values.Callable` rather than reinstating `hashProc`/`equivProc`. Knock-on: correction 3 **withdrawn** — `hashtable-{equivalence,hash}-function` go back to Go (the design's original filing), since `sealedPrimitive` is the resolution the correction claimed was unavailable; this deletes the `%hashtable-kind` seam whose only consumer was those two, and a type error now names the procedure the caller wrote. Only `hashtable-update!` stays Scheme, because it *calls* its argument |
...
### Standard Library
- [ ] **Hashtables → R6RS `(rnrs hashtables)`** [M/L]: The current 10-primitive API conforms to no standard — R6RS *spelling* with neither R6RS nor SRFI-69 semantics (`make-hashtable` nullary vs R6RS's required hash+equiv; `hashtable-ref` default optional vs required; `hashtable-keys` returns a list vs a vector; no `-contains?`/`-update!`/`-entries`/`-mutable?`; a non-R6RS `hashtable-values`). **Decision taken: R6RS**, not SRFI-125 — it keeps the `hashtable-*` spelling already in the tree and is a ratified standard rather than a superseded SRFI. The migration's spine is one inversion: the hash moves from the KEY (`values.Hashable.HashCode`, which gates admission) to the TABLE (a constructor argument), which is what makes pair and vector keys legal and retires both the e-graph's injective-encoding workaround (above) and `graph.scm:366-380`'s raise. 12 R6RS additions plus one extension, 5 signature changes, 1 deletion, 1 arity collision; **15** stdlib + **47** Go-test `(make-hashtable)` call sites (0 in `test/`) — earlier drafts of both plans said 22/2/49, which counted bare string *mentions*, and 14 of those are comments a `sed` cannot reach. Two blocking forks: **Q1** custom hash/equiv procedures (Go→VM sub-context callback, which truncates captured continuations, vs a Scheme-record table — recommendation is to DEFER Phase 4 and ship the eq/eqv/equal surface first) and **Q4** `equal?` on two hashtables once container keys can nest (today structural by deliberate choice, `docs/reference/r7rs-differences.md:22`). Note the design corrects a stale rationale: `TestNoContainerIsHashable`'s comment blames Go-stack recursion in `Hashtable.EqualComponents`, but `(*Pair).EqualTo` delegates to the iterative `values.Equal`; the real hazard is a hashtable used as another hashtable's *key*. **Impl plan written 2026-08-03** (14 tasks, all six Qs resolved: Q1 defers Phase 4, Q4 takes the leaf-gate plus a same-kind conjunct the design omits). Verifying the design against HEAD refuted its `EqualHash` construction — an ordinal-keyed visited map gives different hashes to `(1 2 . A)` and `(1 2 1 2 . B)`, which `equal.go`'s coinductive closure calls `equal?`; the plan hashes a budgeted prefix of the *unfolding* instead. **Two residuals filed against the same walk, both consequences of `equal-hash` becoming Scheme-visible.** `*Record` got a structural arm folded into Task 2 rather than deferred — a user-defined record is the likeliest argument to the only procedure in the surface that accepts an arbitrary object, and the opaque type-name bucket made `equal-hash` a constant on it. `*RecordType` did **not**, and stays a one-bucket key on an `equal` table: its `equal?` is pointer identity (`record_type.go:115-121`) with no `EqualComponents` to mirror, so the hash carries no information. That degradation is *introduced* here — such keys are rejected outright today — and the obvious repair is barred, since identity-hashing a Scheme-visible hash would put an address into it and stop being reproducible across runs. `make-eq-hashtable` is the documented answer; a deterministic name-and-arity arm is the answer if a consumer appears. Same shape covers ports, procedures, and native errors. `plans/2026-08-03-r6rs-hashtables-design.local.md`, `plans/2026-08-03-r6rs-hashtables-impl.local.md`.
