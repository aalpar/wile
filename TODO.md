...
| `2026-08-01-remove-rwmutex-once-impl.local.md` | **Impl-ready**, decision taken 2026-08-01. Removes `RWMutex` and `Once` from the Scheme surface. **Land before Wave 3 §4 (item 16)** — it moots defects 36/37/39, cutting that item from six defects to three and deleting the `values.Once` rewrite. Note the plan's Phase 3 asserts this index row "is already in (added 2026-08-08)"; it was not, in this file or anywhere in its git history, until this row |
...
| `2026-08-09-wave-impl-sequencing.local.md` | Derived schedule for the seven 2026-08-07 wave/immutability plans plus the rwmutex removal, against HEAD `003b3353`. Not a new argument: every edge is stated in at least one plan, and disagreements between plans are called out and resolved |
