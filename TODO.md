...
| Plan | Status |
|---|---|
| `2026-08-07-review-wave1-silent-corruption-design.local.md` | **Not started.** Wave 1 of the 2026-08-07 review remediation: 6 items, 14 defects (16 with the two, N1/N2, found while resolving Q2), all "correct program, wrong answer, no diagnostic" (`with-syntax`/`syntax-case` frame leak; the three immutable-literal defeats; `=`/`<` not a consistent order; `OpSelfTailCall` and peephole on ≠1 value; `error` missing `InvokesProcedure` plus the unenforced Stable stamp; `syntax-rules` pattern literals). **All three questions resolved 2026-08-07.** Q3: `Stable` is an optimizer hint, not a write barrier; `OpSelfTailCall` gets a runtime guard re-checking its own premise (two conjuncts: not-captured via the `envPooled` analogue, still-immutable via a pinned-binding compare carried on a `NativeTemplate` field). Q1: literal pooling is per-template by design, pinned by a ratchet. Q2: shape B (make `(syntax x)` resolve at compile time so the per-context `syntaxCaseState` slot disappears), and **answering it surfaced two defects the review missed**, both measured against `f68c3431`: `syntax-case` is **not reentrant** (a nested `syntax-case` in a clause body destroys the enclosing form's state — documented at `operation_syntax_case.go:54-59` in a Go comment and nowhere user-facing, and it corrects the review's own scope note claiming the leak only masks already-erroneous code); and **`()` is not a valid `syntax-case` pattern** (`CompileSyntaxPattern: pattern must be a list`, nested or not, while the identical `syntax-rules` macro matches it). Together they make the canonical syntax-list walk — `let loop` plus an inner `syntax-case` with a `()` base case, the shape SRFI-197's reference implementation uses eight times — unusable in Wile, so item 1 is larger than M. Item 4 must precede the queued `OpSelfTailCall` v2 (`TODO.md:2100`). **All three remaining §8 questions DECIDED 2026-08-08.** Q3 is answered on the *proof* side rather than the consumer, retiring the (a)/(b)/(c) arity-gate fork instead of choosing among it: `Stable` is stamped only where the writer set is closed and known (locals, sealed coordinates — not mutable top-level defines), the three ungated `DefineOwnGlobal` writers refuse a rebind (`Engine.Define`, `Engine.RegisterPrimitive`, `namespace-define!` — the last Scheme-reachable at runtime, since `(interaction-environment)` IS the engine root), imports relocate to **T2** so a top-level define shadows rather than reassigns, and the `Imported` arm of `IsStable()` therefore stays sound. Price accepted: the frame-reclaim payoff on user top-level defines. Q4: two sequenced branches. Q5: price the prompt-frame suspend/resume option first (Wave 3 Q5 owns it), which if it holds removes §4.5's disarmed-measurement caveat entirely. **§2.3's shape D is superseded** — its ordering fix for 16b is replaced by marking the two literal appenders that never mark, see the immutability row |
