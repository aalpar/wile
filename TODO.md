...
The 2026-08-24 archive pass did the same thing to four more, all of them cited
only inside the 2026-08-21 tier-1 remediation row:

| File | Closed by |
|---|---|
| `pkg/machine/peephole.go` | R1 — the promoted-operator fusion checked the *promoted* arity against the call site, so a wrong-arity call could fuse; the guard now reads the underlying primitive's |
| `pkg/wile/dialect.go` + `pkg/machine/compilation/ref_index.go` | R2 — `ref_index` boxed, so `ForEachOpaqueLiveSymbol` stops handing out a raw index that outlives its table |
| `pkg/security/filesystem_root.go` | R4 — `FilesystemRoot` defaults to deny rather than to the process's root |
| `docs/extensions/architecture.md` | R3's documentation half — the authorizer gates on the **executing** namespace, not the registering one, which is a behaviour break by design (`b4ba36f1`) |
...
**Twenty-one rows left this index on 2026-08-24**, in an archive pass that moved
every plan whose work had landed. Thirty-one files, not twenty-one: four of the
rows named a `-design`/`-impl` pair, and six of the files had no Plan Index row
at all. **`make indexlint` flagged exactly one of that six**
(`2026-08-20-pattern-variables-as-bindings-impl`); the other five passed because
the gate accepts a citation anywhere in this file, and each was named in the body
prose below rather than in the index. Worth knowing before trusting a green
`indexlint` as evidence the index proper is complete — it is a weaker check than
its section heading suggests. What went: the flat-closure arc (design, impl, let-slot
merging, the frame-release design Phase 0 killed, the baseline ledger) plus the
`OpPushEnv` frame-pooling scope it settled; use-site scopes and pattern variables
as bindings; the flat binding model; library phase isolation; strict namespace
levels; immutability representation; free-template-id hygiene; the `#`-reader
rework (model + impl); the N-1 error arc's last two plans; quasi-expand reduction
and its prototype patch; the 2026-08-21 tier-1 remediation; the CLAUDE.local.md
accuracy audit; the climbing tower's Tier-1 impl and its Q4 note; and
`remove-rwmutex-once`. All are in `memory/`, and **each carries its final Plan
Index row verbatim in an ARCHIVED banner** — so the rows are not deleted, they
moved. Three judgment calls the pass made, recorded because nothing else records
them:

- **`2026-07-10-climbing-tower-design.local.md` stays and its `-impl` twin went.**
  Tier 1 shipped (`f92568a8`); Tier 2 (§6) is unstarted and gated on the §9
  procedural-authoring pivot plus §10 owner sign-off. That is the design/impl
  split this preamble warns about, run deliberately instead of by accident, and
  the surviving row says which half is which.
- **Four headers were corrected in place on arrival**, every one of them the stale
  `-design` trap: pattern-variables-as-bindings read "DESIGN — not started" while
  both its `-impl` twins ran to completion; reader-hash-dispatch-model read
  "DESIGN APPROVED, NOT IMPLEMENTED" after all seven phases shipped;
  oppushenv-frame-pooling read "nothing implemented" while its own §8 recorded the
  as-built result; strict-namespace-levels read "DESIGN. Nothing here is built." /
  "READY, ungated" against a row saying ALL 7 PHASES SHIPPED. Each keeps its
  superseded text quoted beside the correction. Ten archived `-impl` plans carry
  unticked checkboxes; in every case the row or the banner names the landing
  commits, and the boxes are bookkeeping this repo has never kept.
- **Nothing partial moved.** Extension contracts, sec4 AU.1, bootstrap-core
  unification, scope-keyed tier-1, the a-local capture proof, name-keyed residuals
  Branch B, frame-reclaim Phase C, go-volume-reduction Phases 2–3, TECH-DEBT 6.2,
  the callback-conditional capture arc, complexity-inventory-refactor, and the
  `-design` notes with no written `-impl` all stay below.

**Eight rows left this index on 2026-09-12**, in an archive pass run on the same
rule as the 2026-08-24 one: move every plan whose work has landed, nothing
partial. Eight rows, eight files — no row named a pair, and every one of the eight
had a row, so `make indexlint`'s plans direction stayed at its one pre-existing
offender (`2026-09-06-scheme-specified-syntax-forms-review`, which has never had a
row) rather than improving. What went: the **Flatt binding model Stage A**
design + impl and **Stage B** design + impl, the whole **name-keyed identity
residuals** trio (finding report, resolved design, implementation), and the
**layered environment architecture** note. All eight are in `memory/`, indexed by
`memory/INDEX-environment-and-phases.local.md`, and each carries its final Plan
Index row verbatim in an ARCHIVED banner — the rows moved, they were not deleted.
Three judgment calls, recorded because nothing else records them:

- **The 2026-08-24 pass got name-keyed residuals wrong, and this one says why.**
  That pass listed "name-keyed residuals Branch B" under *nothing partial moved*.
  Branch B had already shipped — B0+B1+B2 `ab495854`, B3 `0f9b7751`, verified
  ancestors of `master` on 2026-08-08, sixteen days before the pass — and the
  file's own §Branch B header said so. What the pass read instead was the Plan
  Index row, which still said "Branch B (Finding 1) not started". **A pass that
  trusts the row over the file inherits the row's staleness**, which is the same
  failure mode as the stale-`-design` trap, one level up: the index can be stale
  about an `-impl` too.
- **`2026-06-13-layered-environment-architecture` moved even though its own header
  asks to be kept.** It reads "IMPLEMENTED — retained for the model, not as open
  work", and the model is genuinely still worth reading. But there is no open work
  in it, its `-impl` twin has been in `memory/` since 2026-06-14, and its
  *representation* was superseded by the flat binding model (`dea9bdcc`). Keeping
  the design beside a shipped, archived impl is exactly the design/impl split this
  preamble warns about. `memory/` is not less readable than `plans/`; "retained"
  and "archived" were never in tension.
- **Five headers were corrected on arrival** — the stale-`-design` trap again, and
  every one of them understated rather than overstated: flatt-A-design never said
  it was built; flatt-A-impl stopped at "EXECUTION STARTED … Task 0 COMPLETE";
  flatt-B-design never said its recommendation was adopted; flatt-B-impl still read
  "ready to execute from Task 0" after all nine tasks shipped; name-keyed-impl's
  `STATUS: DONE` covered Branch A only. Each keeps its superseded text quoted
  beside the correction. Unticked `- [ ]` boxes were **not** treated as evidence,
  per the 2026-08-24 pass's own finding that this repo has never kept them.
...
- [ ] **A `let` inside a procedure is not fresh when a continuation re-enters it** [High,
  correctness regression]: `6a017fa1` allocates a `let`'s slots in the enclosing procedure's
...
- [x] **`WithContractEnforcement` is SPLIT across the `WithNamespace` line, and shipped
  ...
  (`7c47889f` typed options, `1a36e8b4` `NewEngineWithNamespace`, `a71c973b` this row,
  `46648aa0` ratchets). Filed [Medium, S, 2026-08-24] as needing a maintainer call, which it
...
- [x] **A library environment now owns a seal, and the arm is phase-relative** [High, M, filed
  ...
  **Mechanism — ONE arm causes both directions, and it is not the parent edge.** Corrected
  ...
  `memory/2026-08-04-library-phase-isolation-design.local.md` §2, which supersedes the
...
- [x] **Decide how a phase-0 handler is kept off the value-resolution path** [High, M, filed
  ...
  **The fork is really one question: WHERE does the authority live?** "Is this name a variable or
  ...
  1. **Authority in the FRAME — repair the current model.** Give phase 0 separate value and
     ...
     value walk. **That constraint is weaker than stated:** `LookupSyntaxCompiler` had NO
     production caller (test-only) and was deleted in `c8080848`, so the phase-0
     syntax-compiler bindings serve exactly one consumer today, `findLibraryBinding`. Establish what that consumer needs before assuming
  ...
  **semantically, nothing** — confirmed by the fold: every job the topology used to do now
  ...
  disappears. Design plan: `memory/2026-08-05-flat-binding-model-design.local.md`.
...
- [x] **DECLINED as a product decision, 2026-09-10** (Stage B impl fork E; design §9 Q1).
  ...
  **There are FOUR population mechanisms. The design names two, the impl plan found a third,
  and the table did not add up until the fourth was found.** Re-measured at `5d69a2fd`,
...
- [ ] ~~**Route the library-body import path through the phase-composing install**~~ — the
  ...
  (`memory/2026-09-08-flatt-binding-model-a-impl`, §0.1 Fork 4)]: inside a
...
- [x] ~~**A plain phase-0 import reuses the base's own slot and restamps it `Imported`**~~
  ...
  (`memory/2026-09-09-flatt-binding-model-b-design`, §1 = slice S0); bisected to
...
- [ ] **Sweep the 8 sweepable binding-tier ordinal labels, or stop using them** [Low, M, filed 2026-09-11
  ...
  **This is a KNOWN-DIRTY baseline, not a "probably fine" one.** Four labels were checked
  against the enum at `5d69a2fd`, 2026-09-11, and filed here as "all four wrong". **Two of
  ...
  | Site | Says | Verdict, re-checked at `ff7f8534` |
  |---|---|---|
  ...
  | `pkg/registry/apply.go` (`registerCompileTimeBinding` doc) | "the same T1 > T2 order" | **WRONG — FIXED in `ff7f8534`** (names `tierExactMutable > tierExactSealed`) |
  | `pkg/registry/apply.go` (same doc) | the ambient tier as "the ranked probe's T3" | **WRONG — FIXED in `ff7f8534`.** The ambient tier is DELETED and T3 now names a live tier that is not the one meant |
  ...
  `ff7f8534` also corrected six labels in `literal_probe_ambient_test.go`,
  ...
  **Filed, not fixed — `pkg/wile/library_export_scope_test.go:153`.** "ambient
  ...
  identifier's neighbours), measured at `5d69a2fd`:

      git grep -oE '[^A-Za-z_]T[0-9]' 5d69a2fd -- '*.go'
...
- [x] **`TestPhase1BaseImportMasksNoPhaseRow`'s non-vacuity floor has 20 names of
  headroom** [measured 2026-09-10]: `len(before)` is **40** at `c28616c1` (read by
...
`c28616c1`**; where the design or the impl plan carried a figure, both values are
...
> optimistic.
>
> One "correction" was itself wrong and was corrected back in `5d69a2fd`: a record task
...
- [x] **The S4 inventory, as a price rather than a plan.** The impl plan's figures are
  all low; measured at `c28616c1`, non-test, tree-wide:
...
- [x] **`CompileSymbol`'s definition-time pin outranks the scoped *global* match** [Correctness, S,
  ...
  **Report correction**: the "user `rec` defined after the expansion" variant was filed as a separate
  ...
  `memory/2026-07-29-name-keyed-identity-residuals-{design,impl}.local.md`.
...
- [x] **A caught compile failure's `error-object-message` is the entire wrap chain, not the
  ...
  *outer* wrap; `open-input-file` keeps `no such file or directory` only because the OS error
  ...
  Plan: `memory/2026-08-15-condition-message-split.local.md`.
...
- [x] **`--cover` writes nothing when the program calls `(exit)`, so `cover-scm` sweeps one file** [Tooling, S, RESOLVED 2026-09-04]: fixed on `fix/profile-flush-on-exit` by neither option below. `extensions/system.SetExitHook` registers a process-wide function that `exitWithCode` runs before `os.Exit` (for both `exit` and `emergency-exit`: the hook is the host's, not the dynamic-wind cleanup R7RS lets emergency-exit skip), and `cmd/wile/main.go` folds all five writers (`--cpuprofile` stop+close, `--memprofile`, `--mutexprofile`, `--blockprofile`, `--cover`/`--cover-summary`) into one `sync.Once`-guarded `writeRunReports`, reached by the deferred call, the explicit end-of-main call, and the hook. Option (b) was rejected: the `process:exit` gate fires before the status argument is parsed, so the code would be lost, and a refused exit is catchable by `guard` and unwinds `dynamic-wind`, so `(exit)` would behave differently only under profiling flags. The `Fail`/`fail` paths were closed the same day on `fix/cover-library-bodies`: `system.Exit(code)` is now the one path to `os.Exit` (hook first); in `cmd/wile` every exit goes through `exitUnarmed` (above the hook registration, nothing open) or `exitArmed` (`system.Exit`; `fail`, `Printf`, and everything after); the CLI guard became a CAS because a report writer failing through the hook re-enters it. Pinned by the `program raises uncaught error` row of `TestCLI_CoverFlag_WritesGoCoverFormat`. The `prim_system.go` line shift was absorbed by regenerating `testdata/axis-b-manifest.scm` (`WILE_AXIS_B_UPDATE=1`), which also rewrote 118 masked go-function rows. Pinned by `cmd/wile/cover_integration_test.go` (`(exit 0)`, `(exit 3)`, `--cpuprofile`) and `extensions/system/prim_system_test.go` `TestExitHook`. **Status 2026-09-04:** `make lint && make covercheck` green on the branch, uncommitted; `build/scheme-coverage.out` now spans 68 files (63 suites plus macro attributions), up from 1. Five `testdata/axis-b-manifest.scm` rows moved with the shifted `prim_system.go` primitives (patched by hand, not regenerated: a regen also rewrites 118 inlining-dependent function-name rows). What the sweep still cannot see is the next item. Original analysis: `cmd/wile/main.go` writes the coverage profile after the run returns (`writeCoverageFile`, "written at exit after all work is done"), but `exitWithCode` in `extensions/system/prim_system.go` calls `os.Exit` directly, so the write is never reached. Every real suite ends with `(test-exit)`, which is `(exit 0)`/`(exit 1)`, and `tools/sh/run-all.sh` refuses a suite without it — so `tools/sh/cover-scm.sh`, which runs every `*-test.scm` with `--cover --cover-stdlib` and merges the results, produces a profile of exactly one file: `pkg/stdlib/lib/wile/algebra/sat-test.scm`, the library-body test that never calls `test-exit`. Measured: `./dist/wile --quiet --cover c.out --cover-stdlib -f test/scheme/lists-test.scm` exits 0 and creates no file; the same command on a four-line suite without `(test-exit)` writes a 10-line profile; `build/scheme-coverage.out` after `make covercheck` has 146 entries, all `sat-test.scm`. The `covercheck` prerequisite has looked like a suite-wide Scheme sweep while measuring one file. Was not a gate at the time (the 80% threshold read the Go profile only); `make covercheck` now runs `covercheck.sh` on the Scheme profile too, per library directory with suites skipped, since 2026-09-04 on `fix/cover-library-bodies`. Excluded below the line that day: `chibi` 79.8%, `srfi/1` 66.2%, `srfi/132` 14.2% (open item below). **Fix options:** (a) flush the profile in a path `os.Exit` cannot skip — register the write as an exit hook the exit primitive runs, or write incrementally; (b) when `--cover` is set, install the same `process:exit` denial the in-process driver uses (`denyProcessExit` in `test/scheme_inprocess_test.go`) and turn the refused exit into the process status. (b) reuses an existing seam and also fixes the other profiles lost the same way: `--memprofile`, `--mutexprofile`, `--blockprofile` are written in the same post-run block, and `--cpuprofile` is finalized by a `defer pprof.StopCPUProfile()` that `os.Exit` skips, leaving an unflushed file. Pin with a `cmd/wile/cover_integration_test.go` case whose program calls `(exit 0)`.
- [x] **`--cover` never instruments imported library bodies, so `--cover-stdlib` surfaces nothing but macro attributions** [Tooling, S, RESOLVED 2026-09-04]: fixed on `fix/cover-library-bodies` with a second registry hook, not by wrapping the library callback. `LibraryRegistry.SetCompileObserver` takes a `func(*CompiledLibrary)` that `compileAndExecuteLibrary` (`library_loader.go`) fires after the name check and **before** `EvalTemplate`; `setupLibrarySystem` installs `trackTemplateTree(col, lib.Template)` there when the engine has a collector, taking the collector as a parameter the way it takes `importObserver`, because the engine struct is not built until after setup returns. **Wrong seam, recorded so the next reader does not reach for it:** the existing import observer fires after `LoadLibrary` returns (`compile_import.go`), but the VM records a hit only when the `executed` bitmap already exists (`machine_context.go`, the `mc.template.executed != nil` guard), and `Collector.Track` is what allocates it, so instrumenting from the import observer would count lambda bodies and report every top-level library form as unexecuted. `compiledLib.SourceFile` moved above the execute so the observer sees a complete record. A library compiles once per registry, so tracking at compile time covers every importer, cached or not. Measured after: the `-L` case below yields the library file with the called body at 1 and the uncalled body at 0; `(import (srfi 1))` under `--cover-stdlib` yields 1,530 `srfi/1.sld` rows; `build/scheme-coverage.out` after `make covercheck` spans **116 files** (was 68). Pinned by `cmd/wile/cover_integration_test.go` `TestCLI_CoverFlag_TracksImportedLibraryBodies` (the `-L` pin) and `TestCLI_CoverStdlib_GatesEmbeddedLibraryBodies` (stdlib rows appear only under `--cover-stdlib`, so the default filter still holds), plus `TestLibraryRegistryCompileObserver`. Gates: `make lint && make covercheck` green. Original analysis: `trackTemplateTree` (`pkg/wile/engine.go`) is called from the two program compile paths only (`:731`, `:1314`). A library compiled by `import` reaches `libraryCallback` (`pkg/machine/compilation/compile_library_forms.go:103`, `:153`) with its body in `CompiledLibrary.Template` (`library_registry.go:101`), and nothing tracks it. Measured: a disk library `(my lib)` exporting `f` and `g`, imported and called from a program run with `--cover --cover-stdlib -L`, yields a profile holding only the program's own lines; `f` and `g` appear nowhere, not even at count 0. The stdlib has 44 `.sld` files with `(begin …)`/`(include …)` bodies; after the exit-hook fix above the full sweep lists 68 files, 63 of them the suites themselves, and the stdlib presence is `chibi/test.sld` plus two `wile/algebra/*.scm`, all macro-template positions attributed back to the library when `(test …)` expands inside a suite (the behaviour `docs/coverage/scheme-coverage.md` documents), not library definitions. So `--cover-stdlib` today only un-filters those attributions, and its "default excludes scheme/, wile/, srfi/" wording implies coverage that was never collected. **Fix sketch:** have the engine install (or wrap) the library callback to `trackTemplateTree(p.coverageCollector, lib.Template)`; `trackTemplateTree` already walks child templates. `SetLibraryCallback` has no caller in `pkg/wile`, so check who consumes it before wrapping. Pin with a `cover_integration_test.go` case that imports a library from `-L` and asserts the library file appears with the called definition at count 1 and the uncalled one at count 0.
...
- [ ] **Unscheduled design-only notes** [Refactor/architecture]: engine-services generic keyed slot
  ...
  architecture direction (`memory/2026-06-13-layered-environment-architecture.local.md`),
...
- [x] **Reader fixes** [Done 2026-07-31, branch `feat/reader-hash-dispatch`]:
  ...
  `memory/2026-07-31-reader-hash-dispatch-model.local.md`. Phased implementation
  and the three decisions (all resolved as recommended — D1-a, D2-a, D3-a):
  `memory/2026-07-31-reader-hash-dispatch-impl.local.md`. The tests that were RED
