...
- [ ] **`PrimitiveSpec` capture-safety capability field** [Performance, M, follow-up to escape-frame-validation]: The escape-frame classifier's Layer C (`internal/validate/frame_reclaim_build.go`) decides whether a *primitive* callee is capture-safe via a hand-maintained, sound-by-default name whitelist (`captureSafePrimitiveNames` — `+`, `cons`, `<`, … contribute no edge; unlisted ⇒ unsafe). A primitive is capture-*unsafe* iff it can invoke a Scheme procedure that captures (`apply`, `map`, `for-each`, `call-with-values`, `dynamic-wind`, `with-exception-handler`, `sort`, …). The principled replacement is a `PrimitiveSpec.InvokesProcedure` (or `CapturesContinuation`) capability field so each primitive — including extension primitives — self-declares, instead of a central name list that silently under-covers extensions. **Gate:** only worth building once the sibling escape-gated plan's Phase 2 measurement shows the classifier's precision matters (the whitelist is sound regardless; this is a coverage/scalability dial). Weight false-positives (declaring a capturing primitive safe) as unacceptable per `feedback_annotation_stability.md`. Q-1 of `plans/2026-06-12-escape-frame-validation-impl.md`.
- [ ] **`markCaptured`/`.Captured` is dead code — delete or unify** [Tech debt, S, follow-up to escape-frame-validation]: `markCapturedBindings` (`internal/validate/validate_capture.go`) writes `ValidatedLetBinding.Captured`, which has **zero production readers** (only `validate_capture_test.go` asserts it) — the B1 consumer from PR #604 never landed (it's the unbuilt half of the B3 item above). `markEscaped`/`.Escapes`, by contrast, is live (read at `compile_let.go:240` for let-lambda inlining). Surfaced while building the escape-gated `FrameReclaimable`/`MayCapture` classifier (`plans/2026-06-12-escape-frame-validation-impl.md`), which is *additive* and deliberately does not touch these. **Decision needed:** (a) delete `markCaptured`+`.Captured`+its tests as dead since PR #604, or (b) keep, and unify all three capture/escape predicate-layers (markCaptured, markEscaped, the new fail-safe `MayCapture`) onto the single `WalkBindingRefs` traversal they already share — gated on whether B1/B3 ever gets a consumer. Folding the fail-safe predicate in with the two best-effort ones risks regressing the inlining contract (`compile_let.go:240`), so unify only with a substitution check across all three call sites.
