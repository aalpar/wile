...
- [ ] **List primitive opcodes** [Performance, M]: Promote `car`, `cdr`, `null?`, `cons`, fixnum `+`, fixnum `<` from ForeignFunction dispatch to VM opcodes (`OpCar`, `OpCdr`, `OpNullQ`, `OpCons`, `OpFixnumAdd`, `OpFixnumLT`). These are the hot-path operations in list iteration — each currently pays full ForeignFunction overhead (argument extraction from MachineContext, type assertion, result boxing). As VM opcodes they'd execute inline in the `Run()` loop. Motivation: Phase 3 of native forms migration showed 6 list algorithms are 4-9× slower in Scheme because these sub-operations are ForeignFunction calls; reducing per-element cost could bring Scheme implementations within the 20% gate and unblock migration. Also subsumes the narrower CxR opcode idea. See `plans/NATIVE-FORMS-MIGRATION.md`.
