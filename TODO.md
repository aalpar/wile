...
**Last Updated**: 2026-07-01 (marked done: Scheme-side line coverage, `PrimitiveSpec` capture-safety field / PR #776, Task 6.4 typeswitchlint drift-guard — all verified shipped on master)
...
- [ ] **D2 — lock asymmetry on thread-shared global** [Low, M, pre-existing — DEFERRED 2026-06-15 to its own PR]: out-of-diff root cause (`environment/binding.go`), present before this branch; a real `-race` fix touches the binding read/write hot path and deserves dedicated benchmarking, not a bundle into the layered-environment branch. `environment/binding.go` `Value()`/`SetValue()` are unsynchronized while `set!` writes lock the frame mutex; the diff makes the thread-shared mutable global the explicit default and relocates mutable port parameters into the shared sealed base. `-race` passes only because the new thread test uses `thread-join!` (happens-before); an un-joined concurrent reader would race. Pre-existing; out-of-diff root.
