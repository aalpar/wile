...
- [ ] **`read` mid-parse EOF should raise read-error, not return EOF** [Medium, S]: Found during Phase 4 exceptions audit (`plans/2026-04-19-audit-findings-phase4-exceptions.md` G.1). `(read (open-input-string "("))` returns `#!eof` instead of raising a `read-error?`-satisfying exception per R7RS §6.13.2. String reader correctly raises; pair and vector readers don't. Root cause: `internal/parser/parser.go` returns `io.EOF` for both clean and mid-parse EOF; `PrimRead` can't distinguish. Fix: new sentinel for unterminated compound datums, propagate through `readList`/`readVector`/`readByteVector`, wrap as read-error in `PrimRead`/`PrimReadSyntax`/`PrimReadToken`. Documented in `docs/reference/r7rs-differences.md` deviation #5. Estimated 1–2 hours.
- [ ] **Audit PrimitiveSpec `ReturnType` and `ParamTypes` annotations** [High, L]: PR #664 review uncovered two annotations that lied about behavior — `procedure-arity` claimed `TypePair` but returned pair/list/integer/`#f` across branches; `open-output-bytevector`'s one-arg form claimed `TypeBinaryOutputPort` while passing through whatever `OutputPort` the caller supplied (and that arm turned out to be dead, removed in d098c54b). Both were silent until someone looked. Scope: ~421 `ReturnType:` sites across 39 files (all `registry/` + `extensions/` + `internal/extensions/` + `engine.go`), plus matching `ParamTypes` entries. Three comparison axes, each catches different drift:
