...
### Opaque-subtree over-marking may loosen the immutable-top-level check (crosscheck `15b68433..HEAD`, 2026-07-14)
...
- [ ] **`thread-join!` on a terminated but never-started thread blocks forever** [Correctness, S]: Found while probing the SRFI-18 surface for the `Terminate` fix above; **pre-existing and untouched by it**. `Thread.Terminate` on a `ThreadNew` thread stores the terminated-thread exception and sets `state = ThreadTerminated`, but `done` is closed only by the goroutine `Thread.Start` spawns — which never ran, and now never can (`Start` rejects `state != ThreadNew` with `ErrThreadAlreadyStarted`). So `Thread.Join(nil)` parks on `<-p.done` forever; with a timeout it raises `JoinTimeoutException` instead of the stored exception. Verified: `(let ((th (make-thread (lambda () 42)))) (thread-terminate! th) (thread-join! th 1))` → `thread-join!: timeout`. SRFI-18 says joining a terminated thread raises its end-exception, and the exception is *already stored* — only the `done` signal is missing. **Possible actions:** (A) have `Terminate` close `done` when it terminates a never-started thread (it is the terminator, so it owns the signal); (B) have `Join` consult state/outcome before parking. **Recommendation: A** — `done` means "no further outcome is coming", which is exactly what `Terminate` establishes; B leaves `done` lying about a thread that will never signal. Note `prim_threads_test.go`'s existing "terminate thread" case terminates a never-started thread and never joins, which is why this has gone unnoticed.
