...
- [ ] **`(wile algebra incidence)` library** [Algebra, Design locked]: Möbius/incidence algebra on locally-finite posets per Rota (1964). Formalizes ad-hoc direct-vs-transitive handling across four wile-goast posets (dominator trees, subtype lattices, call-graph reachability, import DAGs) and belief-DSL overlap normalization (inclusion-exclusion on subset lattice). Design decisions resolved: Q1=(b) new library, Q2=(b) `<locally-finite-poset>` carrying `(leq? interval-proc)` — honest to math, supports divisor/Young's/subword posets; Q3=(c) ring-parameterized with `(integer-ring)` default matching `polynomial` idiom; Q4=(b) lazy memoization via `equal?`-keyed hashtable on `(cons x y)`; Q5 record-based API only. ~200 LOC, 6-phase commit strategy, ~25 tests. `plans/2026-04-21-incidence-algebra-impl.md`.
- [ ] **Algebra library roadmap** [Directions]: `plans/2026-04-17-algebra-foundations-directions.md` identifies 6 prioritized directions extending `(wile algebra ...)`. §5.1 `(wile algebra matrix)` shipped via Path D (PRs #684–#691, #695, #696). §5.2 Möbius / incidence algebra — plan locked, tracked separately above. §5.3 AC-matching remains the accuracy prize for wile-goast's `symbolic.scm` / `rewrite.scm` (replaces exponential `discover-equivalences` workaround with polynomial AC-match; now unblocked by §5.1). §5.4–§5.6 (Burnside, free structures, combinatorial graphs) remain funding-gated.
