...
| `2026-08-04-strict-namespace-levels-design.local.md` | Design proposed, 7 tasks, **not started**. Level 2 ("true-empty" visible surface) plus the three §9 leftovers from the shipped level-1 design (`memory/2026-06-27-strict-namespace-mode-*`). Unblocked by `machine.PrimitiveIdentity`. Floor measured, not assumed: phase handlers (`lambda`/`let`/`if`/`import`) survive an empty registry, bootstrap macros (`cond`/`and`/`case`) do not. **Q1 blocking** — level 2 forces every program through a library import, measured at ~10 ms / ~4.8 MB for `(scheme base)` alone, so it ships either before or after the library-env un-copying fix |
