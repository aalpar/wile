...
- [ ] **§5.4 Group actions & Burnside** [Algebra, wile-goast, High]: `(wile algebra group-action)`. Orbit counting and canonicalization under group actions. Formalizes three currently-unnamed special cases in wile-goast: `goastssa/prim_canonicalize.go` (register-renaming action), `ssa-normalize.scm` `ssa-rule-commutative` (S_2 on binops), `boolean-simplify.scm`. Highest "unnamed special case" density of Tier A. No plan file yet. `plans/2026-04-17-algebra-foundations-directions.md` §5.4.
- [ ] **§5.5 Distributive/modular lattice + Birkhoff** [Algebra, wile-goast, Matching, High]: `distributive?` predicate, Birkhoff representation. Certifies MOP=MFP for `dataflow.scm` `run-analysis`; formalizes `domains.scm` precision claims (constant-prop is *not* distributive and silently loses precision at joins). Also substrate for Conway (1976) stable-matching lattice — prerequisite for Tier B. No plan file yet. `plans/2026-04-17-algebra-foundations-directions.md` §5.5.
- [ ] **§5.6 Combinatorial graph** [Algebra, wile-goast]: New `(wile algebra combinatorial-graph)` — distinct from `graph.sld`. Graph isomorphism (enables CFG-level clone detection for `unify.scm`), chromatic/Tutte polynomials, spanning trees, maximum matching. ~600–800 LOC. No plan file yet. `plans/2026-04-17-algebra-foundations-directions.md` §5.6.
- [ ] **§2.2 Free Boolean algebra on atoms** [Algebra, wile-goast, S]: Formalizes `boolean-simplify.scm` normalization — currently works without stating what's being quotiented. ~30 LOC for free-monoid substrate; Boolean case leverages existing `boolean.sld`. Not in §5 prioritization but operationally equal to Tier A. `plans/2026-04-17-algebra-foundations-directions.md` §2.2.

#### Tier B — Two-sided matching (Roth-Sotomayor)

- [ ] **`(wile algebra matching)` library** [Algebra, Matching]: Two-sided matching per Roth & Sotomayor (1990). Gale-Shapley deferred acceptance, Irving rotations, assignment game core (Shapley-Shubik), many-to-one (hospital/intern), many-to-many (Kelso-Crawford substitutes). Depends on §5.1 matrix (shipped), §5.5 distributive lattice (Tier A), §5.7 matroids (Tier C). No plan file yet. `plans/2026-04-17-algebra-foundations-directions.md` §4.6.
- [ ] **§4.2 Tropical permanent / Hungarian primitive** [Algebra, Matching]: Named primitive for bipartite assignment (Hungarian O(n³)) for cross-version symbol matching. Substrate shipped via §5.1 matrix; this surfaces it as first-class. Consumers: `unify.scm` + `fca.scm` already provide similarity scoring. `plans/2026-04-17-algebra-foundations-directions.md` §4.2.
