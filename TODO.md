...
- [ ] **`floor/`, `truncate/` and the `*-quotient`/`*-remainder` family overflow int64 on large exact operands** [Correctness, M — parallel to the BigComplex rounding fix, found by crosscheck consistency lens]: `realDivision` (`extensions/math/prim_rounding.go`, ~line 179) computes the exact-integer path as `int64(roundFn(n0/n1))` on float64s extracted via `helpers.ExtractReal` — the *same* float64 round-trip + int64-cast overflow class just removed from `floor`/`ceiling`/`truncate`/`round` on Rationals. `(floor-quotient <big> <big>)` on operands beyond float64's exact range silently rounds/overflows. The two families differ (division consumes two reals), so it was left out of the `#679` batch. Fix shape: route the exact path through `big.Int` (Euclidean `DivMod` for floor, `QuoRem` for truncate) mirroring `roundRatToInt`, and preserve the multi-value return contract. Guard with large-operand Scheme tests.
- [ ] **BigComplex `angle`/`Phase` big-precision atan2** [Correctness, L — split from BigComplex precision-loss, site 4]: `(*BigComplex).Phase()` (`pkg/values/big_complex.go`, ~line 489) returns a `*BigFloat` but computes `math.Atan2` on `Float64Truncated()` components — cargo-cult precision (the type carries it, the value doesn't). Correct value at normal scale, but wrong when components exceed the float64 range with a finite ratio (both truncate to `+Inf`, `atan2(+Inf,+Inf)=π/4` regardless of the true angle). Deferred from the sites-1/2/3/5 batch because unlike them it is not algebraic: it needs a genuine big-precision `atan2` (argument reduction + arctangent series / CORDIC) with tolerance-based tests, its own numerical-analysis effort. `PrimAngle`'s BigComplex case (`extensions/math/prim_complex.go`) already delegates to nowhere useful — route it through `Phase()` once `Phase()` itself is fixed (routing alone does not help while `Phase` truncates internally).
...
### Architecture
- [~] **Dialect system** [In progress]: forms layer SHIPPED (SP1 per-engine codegen, `WithDialect`, `DefaultDialect`, `R7RSMinimal`, `R5RSStrict`). Phase A primitive-level control SHIPPED (`PrimitiveRemover` + `BootstrapProcedureRewriter` capabilities; `NoMutation` removes ALL 13 mutators genuinely — mutating `vector-map`/`string-map` swapped for a mutation-free bootstrap fragment; inline-HOF optimizer gated on `requires` so removal deopts cleanly). Phase B R6RS-macro control SHIPPED (R5RSStrict removes `syntax-case`/`with-syntax`/… via `fr.Remove` — they carry compiler FormSpecs, so no expander gate is needed; the design's expander-gate premise was falsified). Library-system control SHIPPED (R5RSStrict rejects `import`/`define-library`/`library`/`export` via `forms.DisableExpandForm` + the expander gate; no phase flag needed — construction expands no library forms and lazy loading is unreachable once `import` is rejected; the D-B2 "bootstrap-coupled" fear was over-cautious). Branch `feat/dialect-reject-library-system`. **Dialect system essentially complete** — forms, macros, primitives, and the library system are all per-engine controllable. NoMutation import-reexpose remains a documented language-surface boundary (dialect ≠ sandbox), not a gap. See `plans/2026-07-0{6,7}-dialect-*.local.md`. `plans/ARCHITECTURE.md`
