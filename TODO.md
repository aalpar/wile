...
| Plan | Status |
|---|---|
| `2026-08-07-review-wave1-silent-corruption-design.local.md` | **Phase 3 (§1) done 2026-08-09** on `fix/w1-syntax-case-relocation`; the other five items not started. Wave 1 of the 2026-08-07 review remediation: 6 items, 11 defects, all "correct program, wrong answer, no diagnostic" (`with-syntax`/`syntax-case` frame leak; the three immutable-literal defeats; `=`/`<` not a consistent order; `OpSelfTailCall` and peephole on ≠1 value; `error` missing `InvokesProcedure` plus the unenforced Stable stamp; `syntax-rules` pattern literals). **All three questions resolved 2026-08-07.** Q3: `Stable` is an optimizer hint, not a write barrier; `OpSelfTailCall` gets a runtime guard re-checking its own premise (two conjuncts: not-captured via the `envPooled` analogue, still-immutable via a pinned-binding compare carried on a `NativeTemplate` field). Q1: literal pooling is per-template by design, pinned by a ratchet. Q2: shape **B with the (frame) relocation**, decided 2026-08-09 — the state hangs off the pattern-variable env frame and is resolved by a nearest-marked-ancestor walk from `mc.env`. **The slot RELOCATES; it does not disappear.** (Earlier wording here said "make `(syntax x)` resolve at compile time so the per-context `syntaxCaseState` slot disappears". That is not implementable: the slot holds `bindings`, `matcher` and `input`, and the last two are irreducibly runtime.) **Answering Q2 surfaced three defects the review missed**, all measured: **N1**, `syntax-case` is not reentrant (nested form destroyed the enclosing form's state) — **FIXED**; **N3**, extent asymmetry, `(syntax a)` lexical vs `(syntax (a ...))` dynamic — **FIXED**; **N2**, `()` is not a valid whole-clause `syntax-case` pattern (`CompileSyntaxPattern: pattern must be a list`; a `()` **sub**pattern is fine) — **filed, not fixed**, see the Tier-1 row. N2 is now the only thing still blocking the canonical syntax-list walk (`let loop` plus an inner `syntax-case` with a `()` base case, the shape SRFI-197's reference implementation uses six times). A fourth, **N4** (a nested compile scope in a clause body loses the clause's pattern variables), is filed beside it. Item 4 must precede the queued `OpSelfTailCall` v2 (`TODO.md:2100`) |
