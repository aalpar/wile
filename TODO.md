...
- [ ] **implement let-syntax** - Implement `let-syntax*`
- [ ] **Reduce number of Go primitives** - Reduce number of Go primtives by seeing which ones can be effecitivly replaced by macros or Native continuations.  Both have the potential to be optimized on the Scheme side - macros via code generation and peephole optimization, or "native" scheme opcode/sources that can be optimized via inlineing.
