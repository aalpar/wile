...
- [ ] **Documentation system** [Tooling, High, Needs Design]: REPL-accessible documentation for all Scheme bindings — Go-defined primitives (already have `PrimitiveSpec` metadata), Scheme-defined procedures (44 migrated + user-defined), special forms, and syntax. Existing infrastructure: `DocProvider` interface, `,doc` REPL command, 305 `Doc` entries across registry/extensions. Gaps: (1) Scheme-defined procedures have no doc metadata, (2) no `apropos`/search from REPL, (3) no cross-reference or topic browsing. Design should survey Guile docstrings (`procedure-documentation`), CL `describe`/`apropos`, and Chicken-doc. Key decision: docstrings in `define` bodies vs external doc registry vs both.
  ...
  - [ ] **Broader `procedure-documentation` coverage** [Unification]: Extend `procedure-documentation` beyond `MachineClosure` to all callable types. Add `Documentable` interface (`Documentation() string`). Propagate `PrimitiveSpec.Doc` to `ForeignClosure` during registry `Apply()`. `CaseLambdaClosure` delegates to first clause or carries its own doc. Unify `,doc` REPL command to use `procedure-documentation` instead of separate `DocProvider` path.
  - [ ] **Scribble-style `@` reader notation** [Reader extension]: Racket-style at-expressions for rich documentation markup. Reader recognizes `@cmd[datum ...]{text ...}` and desugars to S-expressions. Enables structured doc content beyond plain strings. Depends on docstring validation being in the validator layer.
...
- [ ] **Evaluate Need for Primitive Annotation Enforcement** envforcement may not be needed.
