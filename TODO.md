...
- [ ] **Audit PrimitiveSpec `ReturnType` and `ParamTypes` annotations against implementations** [High, L]: PR #664 review uncovered two annotations that lied about behavior — `procedure-arity` claimed `TypePair` but returned pair/list/integer/`#f` across branches; `open-output-bytevector`'s one-arg form claimed `TypeBinaryOutputPort` while passing through whatever `OutputPort` the caller supplied (and that arm turned out to be dead, removed in d098c54b). Both were silent until someone looked. Scope: ~421 `ReturnType:` sites across 39 files (all `registry/` + `extensions/` + `internal/extensions/` + `engine.go`), plus matching `ParamTypes` entries. For each: read every return path, verify the annotation is the narrowest *sound* type, note any dead branches. This becomes load-bearing the moment "Extension API contracts Phase 2+" (Tier 2) ships compile-time checking — unsound annotations will turn into wrongly-rejected programs.
