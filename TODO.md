...
- [ ] **Recover the 1.3% `MachineClosure` widening cost by splitting the legacy in-place closure into its own type** [Performance, S-M, HYPOTHESIS UNTESTED — 2026-08-01]: The closure pair split (`perf(machine,environment): capture closures as shape+parent`) removed an 80-byte frame per evaluated lambda and grew `MachineClosure` 16→24B, because it carries `frame` **and** `parent` where it used to carry one `env`. Measured cost on `BenchmarkParallelScalingCompute` (fib, which never builds a closure in its loop but runs the altered apply path on every recursive call): **+1.25% / +1.56% / +1.17% at P=1/2/4, p=0.002, n=6**; indistinguishable at 8/16 where the spread is ±3%. Deliberately accepted — the same change is −20% to −38% on `…ScalingControl`. Numbers and method are in the `scaling_bench_test.go` header.
  ...
  **The lever.** The two fields exist only to discriminate two representations: `parent != nil` is the OpMakeClosure pair, `parent == nil` is a materialized environment used in place. The second is reachable from just two production sites, `(compile …)` (`extensions/eval/prim_eval.go`) and `machine/util.go`, so it could become a distinct closure type behind the `Closure` interface, letting the common case go back to two words. **Constraint that makes this non-trivial:** the in-place case must read `frame.Parent()` LATE, at apply, not at construction. `(compile …)` captures a frame whose parent is non-nil at capture and nil by the time the thunk runs; a snapshot sends it down the copying branch and it faults on a frame with no locals. That is documented on `MachineClosure` and is the reason the discriminated field exists rather than a constructor-time decision. A type split must preserve it.

  **Gate:** only worth doing if (a) a 16B prototype actually recovers the 1.3% under benchstat, and (b) adding a second `Closure` implementation does not put dynamic dispatch on the apply path — this codebase has a measured preference for switch over table dispatch, and `applyClosure` is the hot path the change is trying to speed up. If (b) eats the gain, close this as WONTFIX and leave the note.
