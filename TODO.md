...
- [ ] **Recover the 1.3% `MachineClosure` widening cost** [Performance, S-M — **STRUCTURE SHIPPED 2026-08-17, PERF STILL UNMEASURED**; filed 2026-08-01 as "…by splitting the legacy in-place closure into its own type", which is not the route taken]: The closure pair split (`perf(machine,environment): capture closures as shape+parent`) removed an 80-byte frame per evaluated lambda and grew `MachineClosure` 16→24B, because it carries `frame` **and** `parent` where it used to carry one `env`. Measured cost on `BenchmarkParallelScalingCompute` (fib, which never builds a closure in its loop but runs the altered apply path on every recursive call): **+1.25% / +1.56% / +1.17% at P=1/2/4, p=0.002, n=6**; indistinguishable at 8/16 where the spread is ±3%. Deliberately accepted — the same change is −20% to −38% on `…ScalingControl`. Numbers and method are in the `scaling_bench_test.go` header.
  ...
  **The perf gate is still open, and the hypothesis it tests looks wrong.** The +1.3% was measured on fib, which — as this entry's own first paragraph says — *never builds a closure in its loop*. Closure size cannot be its cost: the 16B/24B difference is paid at `OpMakeClosure`, which fib's loop does not execute. What fib's loop does run is the apply path, and the split changed that too: `mcls.env` → `InitApplyFrame(env)` became `ApplyParent()` (a branch) + `mcls.frame` + a second call argument. The nil-parent fold deleted the branch; the frame elide swapped `mcls.frame` for `tpl.Shape()`, which is the same load count. **So the size win almost certainly does not show up on fib at all**, and the fold — not this — is the change that plausibly moves it. A/B both against `3df1b514^` on `…ScalingCompute` with `-count 6` + benchstat before crediting either, and record which one moved it.
