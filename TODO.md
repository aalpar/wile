...
- [ ] **Extract `ForeignClosure` type**: The rest-arg buffer optimization (PR #333) creates a behavioral difference at the Apply callsite: foreign closures get buffer-reused rest-args, Scheme closures get fresh allocations. When two things behave differently, the type system should reflect that. Currently distinguished by `atomicBody` flag on `NativeTemplate` — a workaround for the wrong abstraction. The operation level already distinguishes foreign (`OperationForeignFunctionCall`) from Scheme (bytecode via `Run()`); the closure level collapses them into `*MachineClosure`. Extract `ForeignClosure` as a distinct callable type so `ApplyCallable`/`OperationApply` dispatch on it via type switch. `ForeignClosure` could embed `MachineClosure` or share fields. Every site matching `*MachineClosure` needs auditing for whether it should also match `*ForeignClosure`.
- [ ] **`PairBlock` type for `[]Pair`**: `[]values.Pair` appears in `values.List()`, `buildRestArg()`, and block-allocation sites with repetitive linking logic (set car, set cdr to next element, terminate with EmptyList). Extract into a named type with construction and linking methods to consolidate the pattern.
