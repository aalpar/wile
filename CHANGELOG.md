...
- **A `let` binds fresh locations when a continuation re-runs it.** Let-slot
  merging put a `let`'s slots in the enclosing frame, so re-entering the `let`
  through `call/cc` or a composable continuation wrote the slot an earlier
