...
;; Default backstop for the BFS-closure enumerators (subgroup-generated and
;; orbit). INFINITE groups are first-class here — (make-group + 0 -
;; '(generators . (1))) is the integers under +, finitely generated yet
;; infinite — so a closure with no caller-supplied bound would loop forever
;; with no output and no error. This cap turns that silent non-termination
;; into a remedy-pointing error (mirrors dataflow.scm's max-revisits and
;; graph.scm's worklist cap). The contract is termination, not promptness:
;; membership is O(n) per step (setoid-member?/member over `seen`), so on a
;; genuinely infinite structure the cap surfaces only after ~cap^2 work. A
;; caller who wants a fast, precise failure should pass an explicit small
;; (max-size . N); one with a legitimately larger finite structure raises it
;; the same way. 100000 is large enough that no realistic interactive finite
;; enumeration is rejected.
(define %default-closure-cap 100000)

(define (subgroup-generated G generators . opts)
  "Return the subgroup of G generated by GENERATORS via BFS closure under G's\noperation. Optional trailing alist entries:\n  (max-size . N) — abort with an error if the closure exceeds N elements\n\nWith no (max-size . N), a large default cap still bounds the closure, so an\ninfinite or infinitely-generated group raises a remedy-pointing error rather\nthan looping forever.\n\nThe returned group carries the full elements enumeration, G's setoid, G's\nelement? predicate, and GENERATORS as its generating set.\n\nExamples:\n  (group-order (subgroup-generated (cyclic-group 6) '(2)))  => 3\n\nParameters:\n  G : group\n  generators : list\n  opts : alist\nReturns: any\nCategory: algebra\nKeywords: subgroup, closure, BFS, generated subgroup\n\nSee also: `enumerate-finite-group', `subgroup?'."
  (let* ((op       (group-op-fn G))
         ...
         ;; With no caller-supplied max-size, fall back to %default-closure-cap
         ;; so an infinite / infinitely-generated group raises instead of
         ;; hanging. An explicit max-size still wins (and keeps its original
         ;; "exceeded max-size" error for callers using it to bound a search).
         (cap      (or max-size %default-closure-cap)))
    ;; Track closure size as an integer accumulator so the cap check
                    ...
                    ((>= size cap)
                     (if max-size
                         (error "subgroup-generated: closure exceeded max-size"
                                (list 'max-size max-size 'generators generators))
                         (error (string-append
                                 "subgroup-generated: closure exceeded the default cap of "
                                 (number->string %default-closure-cap)
                                 " elements without terminating — the group is likely"
                                 " infinite (e.g. integers under +) or the generators do"
                                 " not generate a finite subgroup. Remedies: pass an"
                                 " explicit (max-size . N) to bound the closure, or only"
                                 " call this on a group known to be finite (finite-group?)")
                                (list 'generators generators 'cap %default-closure-cap))))
                    (else
                     (scan (cdr gs)
                           (cons new-elt seen)
    ...
    (cond
      (gens
       (let* ((inverse  (group-inverse-fn G))
         ...
         (let bfs ((seen (list x)) (frontier (list x)) (size 1))
           (cond
             ...
             (else
              (let ((current (car frontier)))
                (let scan ((gs gens*) (seen seen) (frontier (cdr frontier)) (size size))
                  (cond
                    ((null? gs) (bfs seen frontier size))
                    (else
                     (let ((z (act (car gs) current)))
                       (cond
                         ((member z seen)
                          (scan (cdr gs) seen frontier size))
                         ((>= size cap)
                          ;; Infinite orbit (infinite acting group, e.g. Z) — the
                          ;; frontier never empties. Raise instead of hanging.
                          (if max-size
                              (error "orbit: exceeded max-size"
                                     (list 'max-size max-size 'base-point x))
                              (error (string-append
                                      "orbit: discovered more than "
                                      (number->string %default-closure-cap)
                                      " elements without closing — the orbit is likely"
                                      " infinite (the acting group is infinite, e.g."
                                      " integers under +); orbits are enumerable only"
                                      " when finite. Remedies: act with a finite group,"
                                      " restrict to a finite invariant subset, work"
                                      " through a finite quotient action, or pass an"
                                      " explicit (max-size . N) to bound the search")
                                     (list 'base-point x 'cap %default-closure-cap))))
                         (else
                          (scan (cdr gs)
                                (cons z seen)
                                (cons z frontier)
                                (+ size 1))))))))))))))
      (elts
       ;; The element enumeration is finite, so this branch cannot hang — no
       ;; default cap is applied here (it would false-raise on a legitimately
       ;; large finite group). An explicit (max-size . N) is still honored so
       ;; the option means the same thing on both branches.
       (let iter ((gs elts) (seen '()) (size 0))
         (cond
           ((null? gs) (reverse seen))
           (else
            (let ((z (act (car gs) x)))
              (cond
                ((member z seen) (iter (cdr gs) seen size))
                ((and max-size (>= size max-size))
                 (error "orbit: exceeded max-size"
                        (list 'max-size max-size 'base-point x)))
                (else (iter (cdr gs) (cons z seen) (+ size 1)))))))))
      (else
