...
(define fixpoint
  (case-lambda
    ((L f x)
     ...
     ;; Backstop cap (mirrors dataflow.scm's max-revisits=100000, including its
     ;; strict > so a chain converging at exactly the cap is not false-raised).
     ;; The lattice height is opaque here — f keeps ascending on an
     ;; infinite-height lattice (integers, intervals) and Kleene iteration never
     ;; converges. Turn that silent hang into a remedy-pointing error. Steps are
     ;; O(1), so the cap fires promptly. The 4-arg (fixpoint L f x FUEL) form
     ;; below opts out.
     (let ((max-iterations 100000))
       (let loop ((current x) (steps 0))
         (when (> steps max-iterations)
           (error (string-append
                   "fixpoint: exceeded " (number->string max-iterations)
                   " iterations without converging — the lattice likely has"
                   " infinite ascending chains (e.g. integers or intervals) and f"
                   " keeps ascending. Remedies: use (fixpoint/widen L f x WIDEN)"
                   " with a chain-finite widening operator, or the 4-arg"
